...
! ------------------------------------------------------------------------------  
! File created at 10:36 on 5.3.2019   
! ----------------------------------------------------------------------  
 
 
Module LoopMasses_UNHMSSM 
 
Use Control 
Use Settings 
Use Couplings_UNHMSSM 
Use LoopFunctions 
...
Use StandardModel 
Use Tadpoles_UNHMSSM 
 Use EffectivePotential_UNHMSSM 
 Use Pole2L_UNHMSSM 
...
Real(dp), Private :: MChi_1L(6), MChi2_1L(6)  
Complex(dp), Private :: ZN_1L(6,6)  
...
& me32,ms2,ml12l32,M1,M2,M3,M4,Tdp,Tep,kont)

Implicit None 
Real(dp),Intent(inout) :: g1,g2,g3,gp,lam,Tlam,ml32,mHd2,mHu2,me32,ms2,ml12l32(2)
...
& cplAhAhUSvcUSv(3,3,3,3),cplAhAhVZpVZp(3,3),cplAhAhVZVZ(3,3),cplAhAhVZVZp(3,3),         & 
& cplAhcUHpmVWm(3,2),cplAhhhVZ(3,3),cplAhhhVZp(3,3),cplAhHpmcUHpm(3,2,2),cplAhHpmcVWm(3,2),& 
& cplAhSdcUSd(3,6,6),cplAhSecUSe(3,6,6),cplAhSucUSu(3,6,6),cplAhUhhVZ(3,3),              & 
& cplAhUhhVZp(3,3),cplcChaChaUAhL(2,2,3),cplcChaChaUAhR(2,2,3),cplcChaChaUhhL(2,2,3),    & 
& cplcChaChaUhhR(2,2,3),cplcChaChaVPL(2,2),cplcChaChaVPR(2,2),cplcChaChaVZL(2,2),        & 
& cplcChaChaVZpL(2,2),cplcChaChaVZpR(2,2),cplcChaChaVZR(2,2),cplcChacUFuSdL(2,3,6),      & 
& cplcChacUFuSdR(2,3,6),cplcChacUFvSeL(2,3,6),cplcChacUFvSeR(2,3,6),cplcChaFdcUSuL(2,3,6),& 
& cplcChaFdcUSuR(2,3,6),cplcChaFecUSvL(2,3,3),cplcChaFecUSvR(2,3,3),cplcChaUChiHpmL(2,6,2),& 
& cplcChaUChiHpmR(2,6,2),cplcChaUChiVWmL(2,6),cplcChaUChiVWmR(2,6),cplcFdFdUAhL(3,3,3),  & 
...
& cplcFdFdVZpR(3,3),cplcFdFdVZR(3,3),cplcFdGluSdL(3,6),cplcFdGluSdR(3,6),cplcFdUChiSdL(3,6,6),& 
& cplcFdUChiSdR(3,6,6),cplcFeFeUAhL(3,3,3),cplcFeFeUAhR(3,3,3),cplcFeFeUhhL(3,3,3),      & 
& cplcFeFeUhhR(3,3,3),cplcFeFeVPL(3,3),cplcFeFeVPR(3,3),cplcFeFeVZL(3,3),cplcFeFeVZpL(3,3),& 
& cplcFeFeVZpR(3,3),cplcFeFeVZR(3,3),cplcFeUChiSeL(3,6,6),cplcFeUChiSeR(3,6,6),          & 
& cplcFuFdcUHpmL(3,3,2),cplcFuFdcUHpmR(3,3,2),cplcFuFdcVWmL(3,3),cplcFuFdcVWmR(3,3),     & 
& cplcFuFuUAhL(3,3,3),cplcFuFuUAhR(3,3,3),cplcFuFuUhhL(3,3,3),cplcFuFuUhhR(3,3,3),       & 
...
& cplcFuUChiSuL(3,6,6),cplcFuUChiSuR(3,6,6),cplcFvFecUHpmL(3,3,2),cplcFvFecUHpmR(3,3,2), & 
& cplcFvFecVWmL(3,3),cplcFvFecVWmR(3,3),cplcFvFvVZL(3,3),cplcFvFvVZpL(3,3),              & 
& cplcFvFvVZpR(3,3),cplcFvFvVZR(3,3),cplcFvUChiSvL(3,6,3),cplcFvUChiSvR(3,6,3),          & 
& cplcgAgWmcVWm,cplcgGgGVG,cplcgWmgWmUAh(3),cplcgWmgWmUhh(3),cplcgWmgWmVP,               & 
& cplcgWmgWmVZ,cplcgWmgWmVZp,cplcgWmgZpUHpm(2),cplcgWmgZUHpm(2),cplcgWpCgAcVWm,          & 
& cplcgWpCgWpCUAh(3),cplcgWpCgWpCUhh(3),cplcgWpCgWpCVP,cplcgWpCgWpCVZ,cplcgWpCgWpCVZp,   & 
& cplcgWpCgZcUHpm(2),cplcgWpCgZcVWm,cplcgWpCgZpcUHpm(2),cplcgWpCgZpcVWm,cplcgZgWmcUHpm(2),& 
& cplcgZgWmcVWm,cplcgZgWpCUHpm(2),cplcgZgZpUhh(3),cplcgZgZUhh(3),cplcgZpgWmcUHpm(2),     & 
& cplcgZpgWmcVWm,cplcgZpgWpCUHpm(2),cplcgZpgZpUhh(3),cplcgZpgZUhh(3),cplChaFucUSdL(2,3,6),& 
& cplChaFucUSdR(2,3,6),cplChaFvcUSeL(2,3,6),cplChaFvcUSeR(2,3,6),cplChiChacUHpmL(6,2,2), & 
& cplChiChacUHpmR(6,2,2),cplChiChacVWmL(6,2),cplChiChacVWmR(6,2),cplChiChiUAhL(6,6,3),   & 
& cplChiChiUAhR(6,6,3),cplChiChiUhhL(6,6,3),cplChiChiUhhR(6,6,3),cplChiChiVZL(6,6),      & 
& cplChiChiVZpL(6,6),cplChiChiVZpR(6,6),cplChiChiVZR(6,6),cplChiFdcUSdL(6,3,6),          & 
& cplChiFdcUSdR(6,3,6),cplChiFecUSeL(6,3,6),cplChiFecUSeR(6,3,6),cplChiFucUSuL(6,3,6),   & 
& cplChiFucUSuR(6,3,6),cplChiFvcUSvL(6,3,3),cplChiFvcUSvR(6,3,3),cplcHpmVPVWm(2),        & 
& cplcHpmVWmVZ(2),cplcHpmVWmVZp(2),cplcUChacFuSdL(2,3,6),cplcUChacFuSdR(2,3,6)

Complex(dp) :: cplcUChacFvSeL(2,3,6),cplcUChacFvSeR(2,3,6),cplcUChaChaAhL(2,2,3),cplcUChaChaAhR(2,2,3),& 
& cplcUChaChahhL(2,2,3),cplcUChaChahhR(2,2,3),cplcUChaChaVPL(2,2),cplcUChaChaVPR(2,2),   & 
& cplcUChaChaVZL(2,2),cplcUChaChaVZpL(2,2),cplcUChaChaVZpR(2,2),cplcUChaChaVZR(2,2),     & 
& cplcUChaChiHpmL(2,6,2),cplcUChaChiHpmR(2,6,2),cplcUChaChiVWmL(2,6),cplcUChaChiVWmR(2,6),& 
...
& cplcUFdFdVZL(3,3),cplcUFdFdVZpL(3,3),cplcUFdFdVZpR(3,3),cplcUFdFdVZR(3,3),             & 
& cplcUFdFuHpmL(3,3,2),cplcUFdFuHpmR(3,3,2),cplcUFdFuVWmL(3,3),cplcUFdFuVWmR(3,3),       & 
& cplcUFdGluSdL(3,6),cplcUFdGluSdR(3,6),cplcUFeChaSvL(3,2,3),cplcUFeChaSvR(3,2,3),       & 
& cplcUFeChiSeL(3,6,6),cplcUFeChiSeR(3,6,6),cplcUFeFeAhL(3,3,3),cplcUFeFeAhR(3,3,3),     & 
...
& cplcUFuChiSuL(3,6,6),cplcUFuChiSuR(3,6,6),cplcUFuFdcHpmL(3,3,2),cplcUFuFdcHpmR(3,3,2), & 
& cplcUFuFdcVWmL(3,3),cplcUFuFdcVWmR(3,3),cplcUFuFuAhL(3,3,3),cplcUFuFuAhR(3,3,3),       & 
...
& cplcUFvChiSvL(3,6,3),cplcUFvChiSvR(3,6,3),cplcUFvFecHpmL(3,3,2),cplcUFvFecHpmR(3,3,2), & 
& cplcUFvFecVWmL(3,3),cplcUFvFecVWmR(3,3),cplcUFvFvVZL(3,3),cplcUFvFvVZpL(3,3),          & 
& cplcUFvFvVZpR(3,3),cplcUFvFvVZR(3,3),cplcUHpmVPVWm(2),cplcUHpmVWmVZ(2),cplcUHpmVWmVZp(2),& 
...
& cplcVWmVPVWmVZ3,cplcVWmVPVWmVZp1,cplcVWmVPVWmVZp2,cplcVWmVPVWmVZp3,cplcVWmVWmVZ,       & 
& cplcVWmVWmVZp,cplcVWmVWmVZpVZp1,cplcVWmVWmVZpVZp2,cplcVWmVWmVZpVZp3,cplcVWmVWmVZVZ1,   & 
& cplcVWmVWmVZVZ2,cplcVWmVWmVZVZ3,cplcVWmVWmVZVZp1,cplcVWmVWmVZVZp2,cplcVWmVWmVZVZp3,    & 
...
& cplhhhhUSucUSu(3,3,6,6),cplhhhhUSvcUSv(3,3,3,3),cplhhhhVZpVZp(3,3),cplhhhhVZVZ(3,3),   & 
& cplhhhhVZVZp(3,3),cplhhHpmcUHpm(3,2,2),cplhhHpmcVWm(3,2),cplhhSdcUSd(3,6,6),           & 
& cplhhSecUSe(3,6,6),cplhhSucUSu(3,6,6),cplhhSvcUSv(3,3,3),cplhhVZpVZp(3),               & 
& cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmcVWmVWm(2,2),cplHpmcHpmVP(2,2),cplHpmcHpmVPVP(2,2),& 
& cplHpmcHpmVPVZ(2,2),cplHpmcHpmVPVZp(2,2),cplHpmcHpmVZ(2,2),cplHpmcHpmVZp(2,2)

Complex(dp) :: cplHpmcHpmVZpVZp(2,2),cplHpmcHpmVZVZ(2,2),cplHpmcHpmVZVZp(2,2),cplHpmcUHpmVP(2,2),     & 
& cplHpmcUHpmVZ(2,2),cplHpmcUHpmVZp(2,2),cplHpmcVWmVP(2),cplHpmcVWmVZ(2),cplHpmcVWmVZp(2),& 
& cplHpmSucUSd(2,6,6),cplHpmSvcUSe(2,3,6),cplHpmUSdcHpmcUSd(2,6,2,6),cplHpmUSecHpmcUSe(2,6,2,6),& 
...
& cplSdcSdVPVZ(6,6),cplSdcSdVPVZp(6,6),cplSdcSdVZ(6,6),cplSdcSdVZp(6,6),cplSdcSdVZpVZp(6,6),& 
& cplSdcSdVZVZ(6,6),cplSdcSdVZVZp(6,6),cplSdcSucVWm(6,6),cplSdcUHpmcSu(6,2,6),           & 
& cplSdcUSdVG(6,6),cplSdcUSdVP(6,6),cplSdcUSdVZ(6,6),cplSdcUSdVZp(6,6),cplSdcUSucVWm(6,6),& 
& cplSdUSecSdcUSe(6,6,6,6),cplSdUSucSdcUSu(6,6,6,6),cplSdUSvcSdcUSv(6,3,6,3),            & 
& cplSecHpmcUSv(6,2,3),cplSecSecVWmVWm(6,6),cplSecSeVP(6,6),cplSecSeVPVP(6,6),           & 
& cplSecSeVPVZ(6,6),cplSecSeVPVZp(6,6),cplSecSeVZ(6,6),cplSecSeVZp(6,6),cplSecSeVZpVZp(6,6),& 
& cplSecSeVZVZ(6,6),cplSecSeVZVZp(6,6),cplSecSvcVWm(6,3),cplSecUHpmcSv(6,2,3),           & 
& cplSecUSeVP(6,6),cplSecUSeVZ(6,6),cplSecUSeVZp(6,6),cplSecUSvcVWm(6,3),cplSeUSucSecUSu(6,6,6,6),& 
& cplSeUSvcSecUSv(6,3,6,3),cplSucSucVWmVWm(6,6),cplSucSuVG(6,6),cplSucSuVGVG(6,6),       & 
& cplSucSuVP(6,6),cplSucSuVPVP(6,6),cplSucSuVPVZ(6,6),cplSucSuVPVZp(6,6),cplSucSuVZ(6,6),& 
& cplSucSuVZp(6,6),cplSucSuVZpVZp(6,6),cplSucSuVZVZ(6,6),cplSucSuVZVZp(6,6),             & 
& cplSucUSdVWm(6,6),cplSucUSuVG(6,6),cplSucUSuVP(6,6),cplSucUSuVZ(6,6),cplSucUSuVZp(6,6),& 
& cplSuUSvcSucUSv(6,3,6,3),cplSvcSvcVWmVWm(3,3),cplSvcSvVZ(3,3),cplSvcSvVZp(3,3),        & 
& cplSvcSvVZpVZp(3,3),cplSvcSvVZVZ(3,3),cplSvcSvVZVZp(3,3),cplSvcUSeVWm(3,6),            & 
& cplSvcUSvVZ(3,3),cplSvcUSvVZp(3,3),cplUAhAhhh(3,3,3),cplUAhhhVZ(3,3),cplUAhhhVZp(3,3), & 
& cplUAhHpmcHpm(3,2,2),cplUAhHpmcVWm(3,2),cplUAhSdcSd(3,6,6),cplUAhSecSe(3,6,6),         & 
& cplUAhSucSu(3,6,6),cplUAhUAhAhAh(3,3,3,3),cplUAhUAhcVWmVWm(3,3),cplUAhUAhhhhh(3,3,3,3),& 
& cplUAhUAhHpmcHpm(3,3,2,2),cplUAhUAhSdcSd(3,3,6,6),cplUAhUAhSecSe(3,3,6,6),             & 
& cplUAhUAhSucSu(3,3,6,6),cplUAhUAhSvcSv(3,3,3,3),cplUAhUAhVZpVZp(3,3),cplUAhUAhVZVZ(3,3),& 
& cplUChiChacHpmL(6,2,2),cplUChiChacHpmR(6,2,2),cplUChiChacVWmL(6,2),cplUChiChacVWmR(6,2),& 
& cplUChiChiAhL(6,6,3),cplUChiChiAhR(6,6,3),cplUChiChihhL(6,6,3),cplUChiChihhR(6,6,3),   & 
& cplUChiChiVZL(6,6),cplUChiChiVZpL(6,6),cplUChiChiVZpR(6,6),cplUChiChiVZR(6,6),         & 
& cplUChiFdcSdL(6,3,6),cplUChiFdcSdR(6,3,6),cplUChiFecSeL(6,3,6),cplUChiFecSeR(6,3,6),   & 
& cplUChiFucSuL(6,3,6),cplUChiFucSuR(6,3,6),cplUChiFvcSvL(6,3,3),cplUChiFvcSvR(6,3,3),   & 
...
& cplUhhUhhcVWmVWm(3,3),cplUhhUhhhhhh(3,3,3,3),cplUhhUhhHpmcHpm(3,3,2,2),cplUhhUhhSdcSd(3,3,6,6),& 
& cplUhhUhhSecSe(3,3,6,6),cplUhhUhhSucSu(3,3,6,6),cplUhhUhhSvcSv(3,3,3,3),               & 
& cplUhhUhhVZpVZp(3,3),cplUhhUhhVZVZ(3,3),cplUhhVZpVZp(3),cplUhhVZVZ(3),cplUhhVZVZp(3),  & 
...
& cplUHpmSucUHpmcSu(2,6,2,6),cplUHpmSvcUHpmcSv(2,3,2,3),cplUSdcUSdcVWmVWm(6,6)

Complex(dp) :: cplUSdcUSdVGVG(6,6),cplUSdcUSdVPVP(6,6),cplUSdcUSdVZpVZp(6,6),cplUSdcUSdVZVZ(6,6),     & 
& cplUSdSdcUSdcSd(6,6,6,6),cplUSdSecUSdcSe(6,6,6,6),cplUSdSucUSdcSu(6,6,6,6),            & 
& cplUSdSvcUSdcSv(6,3,6,3),cplUSecUSecVWmVWm(6,6),cplUSecUSeVPVP(6,6),cplUSecUSeVZpVZp(6,6),& 
& cplUSecUSeVZVZ(6,6),cplUSeSecUSecSe(6,6,6,6),cplUSeSucUSecSu(6,6,6,6),cplUSeSvcUSecSv(6,3,6,3),& 
& cplUSucUSucVWmVWm(6,6),cplUSucUSuVGVG(6,6),cplUSucUSuVPVP(6,6),cplUSucUSuVZpVZp(6,6),  & 
& cplUSucUSuVZVZ(6,6),cplUSuSucUSucSu(6,6,6,6),cplUSuSvcUSucSv(6,3,6,3),cplUSvcUSvcVWmVWm(3,3),& 
& cplUSvcUSvVZpVZp(3,3),cplUSvcUSvVZVZ(3,3),cplUSvSvcUSvcSv(3,3,3,3),cplVGVGVG,          & 
...
& ZH,ZN,ZP,ZU,ZUL,ZUR,ZV,ZVL,ZW,ZZ,betaH,vd,vu,vS,g1,g2,g3,gp,Yd,y12,y3,lam,             & 
& Yu,Td,Ty12,Ty3,Tlam,Tu,mq2,ml122,ml32,mHd2,mHu2,md2,mu2,me122,me32,ms2,ml12l32,        & 
& M1,M2,M3,M4,Tdp,Tep,GenerationMixing,kont)
...
vu=(TanBetaQ*vSM)/Sqrt(1 + TanBetaQ**2) 
End if 
Call SolveTadpoleEquations(g1,g2,g3,gp,Yd,y12,y3,lam,Yu,Td,Ty12,Ty3,Tlam,             & 
& Tu,mq2,ml122,ml32,mHd2,mHu2,md2,mu2,me122,me32,ms2,ml12l32,M1,M2,M3,M4,Tdp,            & 
& Tep,vd,vu,vS,(/ ZeroC, ZeroC, ZeroC /))
...
Call CouplingsForLoopMasses(Yd,Td,lam,Tdp,vu,vS,ZD,ZA,g2,Yu,UM,UP,ZUL,ZUR,            & 
& g1,gp,ZN,ZDL,ZDR,g3,pG,vd,ZH,Tu,ZU,ZP,TW,TWp,y12,y3,ZE,ZV,ZVL,ZEL,ZER,Ty12,            & 
& Ty3,Tep,Tlam,cplAhSdcUSd,cplChaFucUSdL,cplChaFucUSdR,cplChiFdcUSdL,cplChiFdcUSdR,      & 
...
& cplUSdcUSdVPVP,cplUSdcUSdcVWmVWm,cplUSdcUSdVZVZ,cplUSdcUSdVZpVZp,cplChiFvcUSvL,        & 
& cplChiFvcUSvR,cplcChaFecUSvL,cplcChaFecUSvR,cplhhSvcUSv,cplSecHpmcUSv,cplSecUSvcVWm,   & 
& cplSvcUSvVZ,cplSvcUSvVZp,cplAhAhUSvcUSv,cplhhhhUSvcUSv,cplHpmUSvcHpmcUSv,              & 
& cplSdUSvcSdcUSv,cplSeUSvcSecUSv,cplSuUSvcSucUSv,cplUSvSvcUSvcSv,cplUSvcUSvcVWmVWm,     & 
& cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,cplcUFvChiSvL,cplcUFvChiSvR,cplcUFvFecHpmL,            & 
& cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvVZL,cplcUFvFvVZR,cplcUFvFvVZpL,  & 
& cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,cplAhSucUSu,cplChiFucUSuL,cplChiFucUSuR,   & 
...
& cplUSucUSuVZpVZp,cplAhSecUSe,cplChaFvcUSeL,cplChaFvcUSeR,cplChiFecUSeL,cplChiFecUSeR,  & 
...
& cplUSecUSeVZpVZp,cplAhAhUhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,      & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,cplcgZgZUhh,cplcgZpgZUhh,      & 
...
& cplcgWmgWmUAh,cplcgWpCgWpCUAh,cplUAhhhVZ,cplUAhhhVZp,cplUAhHpmcHpm,cplUAhHpmcVWm,      & 
& cplUAhSdcSd,cplUAhSecSe,cplUAhSucSu,cplUAhUAhAhAh,cplUAhUAhhhhh,cplUAhUAhHpmcHpm,      & 
& cplUAhUAhSdcSd,cplUAhUAhSecSe,cplUAhUAhSucSu,cplUAhUAhSvcSv,cplUAhUAhcVWmVWm,          & 
& cplUAhUAhVZVZ,cplUAhUAhVZpVZp,cplAhHpmcUHpm,cplAhcUHpmVWm,cplChiChacUHpmL,             & 
...
& cplUHpmcUHpmVPVP,cplUHpmcUHpmcVWmVWm,cplUHpmcUHpmVZVZ,cplUHpmcUHpmVZpVZp,              & 
...
& cplUChiChiVZpL,cplUChiChiVZpR,cplUChiFdcSdL,cplUChiFdcSdR,cplUChiFecSeL,               & 
& cplUChiFecSeR,cplUChiFucSuL,cplUChiFucSuR,cplUChiFvcSvL,cplUChiFvcSvR,cplcChaUChiHpmL, & 
...
& cplcChaUChiVWmR,cplcUChaChaAhL,cplcUChaChaAhR,cplcUChaChahhL,cplcUChaChahhR,           & 
& cplcUChaChaVPL,cplcUChaChaVPR,cplcUChaChaVZL,cplcUChaChaVZR,cplcUChaChaVZpL,           & 
& cplcUChaChaVZpR,cplcUChaChiHpmL,cplcUChaChiHpmR,cplcUChaChiVWmL,cplcUChaChiVWmR,       & 
...
& cplcFvFvVZpL,cplcFvFvVZpR,cplcgWmgWmVZp,cplcgWpCgWpCVZp,cplhhVZpVZp,cplHpmcHpmVZp,     & 
& cplHpmcVWmVZp,cplSdcSdVZp,cplSecSeVZp,cplSucSuVZp,cplSvcSvVZp,cplcVWmVWmVZp,           & 
& cplAhAhVZpVZp,cplhhhhVZpVZp,cplHpmcHpmVZpVZp,cplSdcSdVZpVZp,cplSecSeVZpVZp,            & 
& cplSucSuVZpVZp,cplSvcSvVZpVZp,cplcVWmVWmVZpVZp1,cplcVWmVWmVZpVZp2,cplcVWmVWmVZpVZp3,   & 
& cplAhHpmcVWm,cplChiChacVWmL,cplChiChacVWmR,cplcFuFdcVWmL,cplcFuFdcVWmR,cplcFvFecVWmL,  & 
& cplcFvFecVWmR,cplcgWpCgAcVWm,cplcgAgWmcVWm,cplcgZgWmcVWm,cplcgZpgWmcVWm,               & 
& cplcgWpCgZcVWm,cplcgWpCgZpcVWm,cplhhHpmcVWm,cplhhcVWmVWm,cplSdcSucVWm,cplSecSvcVWm,    & 
& cplAhAhcVWmVWm,cplhhhhcVWmVWm,cplHpmcHpmcVWmVWm,cplSdcSdcVWmVWm,cplSecSecVWmVWm,       & 
...
& MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,cplAhAhUhh,cplcChaChaUhhL,cplcChaChaUhhR,               & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,cplcgZgZUhh,cplcgZpgZpUhh,     & 
& cplUhhhhhh,cplUhhHpmcHpm,cplUhhSdcSd,cplUhhSecSe,cplUhhSucSu,cplUhhSvcSv,              & 
& cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZpVZp,Tad1Loop(1:3))
...
CASE ( 1 , 2 ) 
  ! Make sure that there are no exactly degenerated masses! 
   ...
   y12_saveEP =y12
   where (aint(Abs(y12)).eq.y12) y12=y12*(1 + 2*1.0E-12_dp)
   Yu_saveEP =Yu
   where (aint(Abs(Yu)).eq.Yu) Yu=Yu*(1 + 3*1.0E-12_dp)
   Td_saveEP =Td
   where (aint(Abs(Td)).eq.Td) Td=Td*(1 + 4*1.0E-12_dp)
   Ty12_saveEP =Ty12
   where (aint(Abs(Ty12)).eq.Ty12) Ty12=Ty12*(1 + 5*1.0E-12_dp)
   Tu_saveEP =Tu
   where (aint(Abs(Tu)).eq.Tu) Tu=Tu*(1 + 6*1.0E-12_dp)
   mq2_saveEP =mq2
   where (aint(Abs(mq2)).eq.mq2) mq2=mq2*(1 + 7*1.0E-12_dp)
   ml122_saveEP =ml122
   where (aint(Abs(ml122)).eq.ml122) ml122=ml122*(1 + 8*1.0E-12_dp)
   md2_saveEP =md2
   where (aint(Abs(md2)).eq.md2) md2=md2*(1 + 9*1.0E-12_dp)
   mu2_saveEP =mu2
   where (aint(Abs(mu2)).eq.mu2) mu2=mu2*(1 + 10*1.0E-12_dp)
   me122_saveEP =me122
   where (aint(Abs(me122)).eq.me122) me122=me122*(1 + 11*1.0E-12_dp)
   Tdp_saveEP =Tdp
   where (aint(Abs(Tdp)).eq.Tdp) Tdp=Tdp*(1 + 12*1.0E-12_dp)
   Tep_saveEP =Tep
   where (aint(Abs(Tep)).eq.Tep) Tep=Tep*(1 + 13*1.0E-12_dp)
...
Do i2L = 1, iFin 
Call CalculateCorrectionsEffPot(ti_ep2L,pi_ep2L,vd,vu,vS,g1,g2,g3,gp,Yd,              & 
& y12,y3,lam,Yu,Td,Ty12,Ty3,Tlam,Tu,mq2,ml122,ml32,mHd2,mHu2,md2,mu2,me122,              & 
& me32,ms2,ml12l32,M1,M2,M3,M4,Tdp,Tep,kont)
...
If (NewGBC) Then 
Call CalculatePi2S(125._dp**2,vd,vu,vS,g1,g2,g3,gp,Yd,y12,y3,lam,Yu,Td,               & 
& Ty12,Ty3,Tlam,Tu,mq2,ml122,ml32,mHd2,mHu2,md2,mu2,me122,me32,ms2,ml12l32,              & 
& M1,M2,M3,M4,Tdp,Tep,kont,ti_ep2L,Pi2S_EffPot,PiP2S_EffPot)

Else 
Call CalculatePi2S(0._dp,vd,vu,vS,g1,g2,g3,gp,Yd,y12,y3,lam,Yu,Td,Ty12,               & 
& Ty3,Tlam,Tu,mq2,ml122,ml32,mHd2,mHu2,md2,mu2,me122,me32,ms2,ml12l32,M1,M2,             & 
& M3,M4,Tdp,Tep,kont,ti_ep2L,Pi2S_EffPot,PiP2S_EffPot)
...
mHu22L = mHu2
ms22L = ms2
Call OneLoopSd(g1,g2,gp,Qq,QHd,QHu,Qd,Qs,Yd,Td,lam,mq2,md2,Tdp,vd,vu,vS,              & 
& MSd,MSd2,MAh,MAh2,MFu,MFu2,MCha,MCha2,MFd,MFd2,MChi,MChi2,MGlu,MGlu2,Mhh,              & 
& Mhh2,MSu,MSu2,MHpm,MHpm2,MVZ,MVZ2,MVZp,MVZp2,MVWm,MVWm2,MSe,MSe2,MSv,MSv2,             & 
& cplAhSdcUSd,cplChaFucUSdL,cplChaFucUSdR,cplChiFdcUSdL,cplChiFdcUSdR,cplGluFdcUSdL,     & 
& cplGluFdcUSdR,cplhhSdcUSd,cplHpmSucUSd,cplSdcUSdVG,cplSdcUSdVP,cplSdcUSdVZ,            & 
& cplSdcUSdVZp,cplSucUSdVWm,cplAhAhUSdcUSd,cplhhhhUSdcUSd,cplHpmUSdcHpmcUSd,             & 
& cplUSdSdcUSdcSd,cplUSdSecUSdcSe,cplUSdSucUSdcSu,cplUSdSvcUSdcSv,cplUSdcUSdVGVG,        & 
& cplUSdcUSdVPVP,cplUSdcUSdcVWmVWm,cplUSdcUSdVZVZ,cplUSdcUSdVZpVZp,0.1_dp*delta_mass,    & 
& MSd_1L,MSd2_1L,ZD_1L,kont)

Call OneLoopSv(g1,g2,gp,Ql,QHd,QHu,Qs,ml122,ml12l32,ml32,vd,vu,vS,MFv,MFv2,           & 
& MChi,MChi2,MCha,MCha2,MFe,MFe2,MSv,MSv2,Mhh,Mhh2,MHpm,MHpm2,MSe,MSe2,MVWm,             & 
& MVWm2,MVZ,MVZ2,MVZp,MVZp2,MAh,MAh2,MSd,MSd2,MSu,MSu2,cplChiFvcUSvL,cplChiFvcUSvR,      & 
& cplcChaFecUSvL,cplcChaFecUSvR,cplhhSvcUSv,cplSecHpmcUSv,cplSecUSvcVWm,cplSvcUSvVZ,     & 
& cplSvcUSvVZp,cplAhAhUSvcUSv,cplhhhhUSvcUSv,cplHpmUSvcHpmcUSv,cplSdUSvcSdcUSv,          & 
& cplSeUSvcSecUSv,cplSuUSvcSucUSv,cplUSvSvcUSvcSv,cplUSvcUSvcVWmVWm,cplUSvcUSvVZVZ,      & 
& cplUSvcUSvVZpVZp,0.1_dp*delta_mass,MSv_1L,MSv2_1L,ZV_1L,kont)

Call OneLoopFv(MSv,MSv2,MChi,MChi2,MHpm,MHpm2,MFe,MFe2,MVWm,MVWm2,MVZ,MVZ2,           & 
& MFv,MFv2,MVZp,MVZp2,MCha,MCha2,MSe,MSe2,cplcUFvChiSvL,cplcUFvChiSvR,cplcUFvFecHpmL,    & 
& cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvVZL,cplcUFvFvVZR,cplcUFvFvVZpL,  & 
& cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,0.1_dp*delta_mass,MFv_1L,MFv2_1L,          & 
& ZVL_1L,kont)

Call OneLoopSu(g1,g2,gp,Qq,QHd,QHu,Qu,Qs,lam,Yu,Tu,mq2,mu2,vd,vu,vS,MSu,              & 
& MSu2,MAh,MAh2,MFu,MFu2,MChi,MChi2,MCha,MCha2,MFd,MFd2,MGlu,MGlu2,Mhh,Mhh2,             & 
& MHpm,MHpm2,MSd,MSd2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSe,MSe2,MSv,MSv2,cplAhSucUSu,      & 
...
& cplSucUSuVZ,cplSucUSuVZp,cplAhAhUSucUSu,cplhhhhUSucUSu,cplHpmUSucHpmcUSu,              & 
& cplSdUSucSdcUSu,cplSeUSucSecUSu,cplUSuSucUSucSu,cplUSuSvcUSucSv,cplUSucUSuVGVG,        & 
& cplUSucUSuVPVP,cplUSucUSucVWmVWm,cplUSucUSuVZVZ,cplUSucUSuVZpVZp,0.1_dp*delta_mass,    & 
& MSu_1L,MSu2_1L,ZU_1L,kont)

Call OneLoopSe(g1,g2,gp,Ql,QHd,QHu,Qe12,Qe3,Qs,y12,Ty12,y3,Ty3,lam,ml122,             & 
& ml12l32,ml32,me122,me32,Tep,vd,vu,vS,MSe,MSe2,MAh,MAh2,MFv,MFv2,MCha,MCha2,            & 
& MFe,MFe2,MChi,MChi2,Mhh,Mhh2,MSv,MSv2,MHpm,MHpm2,MVZ,MVZ2,MVZp,MVZp2,MVWm,             & 
& MVWm2,MSd,MSd2,MSu,MSu2,cplAhSecUSe,cplChaFvcUSeL,cplChaFvcUSeR,cplChiFecUSeL,         & 
& cplChiFecUSeR,cplhhSecUSe,cplHpmSvcUSe,cplSecUSeVP,cplSecUSeVZ,cplSecUSeVZp,           & 
...
Call OneLoophh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd22L,mHu22L,ms22L,vd,vu,vS,             & 
& MAh,MAh2,MVZ,MVZ2,MVZp,MVZp2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,              & 
& MFu2,Mhh,Mhh2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,MSv2,               & 
& cplAhAhUhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,cplChiChiUhhL,         & 
& cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,cplcFuFuUhhL,        & 
& cplcFuFuUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,cplcgZgZUhh,cplcgZpgZUhh,cplcgZgZpUhh,      & 
& cplcgZpgZpUhh,cplUhhhhhh,cplUhhHpmcHpm,cplUhhHpmcVWm,cplUhhSdcSd,cplUhhSecSe,          & 
& cplUhhSucSu,cplUhhSvcSv,cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZVZp,cplUhhVZpVZp,             & 
& cplAhAhUhhUhh,cplUhhUhhhhhh,cplUhhUhhHpmcHpm,cplUhhUhhSdcSd,cplUhhUhhSecSe,            & 
& cplUhhUhhSucSu,cplUhhUhhSvcSv,cplUhhUhhcVWmVWm,cplUhhUhhVZVZ,cplUhhUhhVZpVZp,          & 
& 0.1_dp*delta_mass,Mhh_1L,Mhh2_1L,ZH_1L,kont)

If (TwoLoopMethod.gt.2) Then 
Call OneLoopAh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd22L,mHu22L,ms22L,vd,vu,vS,             & 
& TW,TWp,Mhh,Mhh2,MAh,MAh2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,             & 
& MVZ,MVZ2,MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,              & 
& MSv2,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,cplChiChiUAhR,             & 
& cplcFdFdUAhL,cplcFdFdUAhR,cplcFeFeUAhL,cplcFeFeUAhR,cplcFuFuUAhL,cplcFuFuUAhR,         & 
& cplcgWmgWmUAh,cplcgWpCgWpCUAh,cplUAhhhVZ,cplUAhhhVZp,cplUAhHpmcHpm,cplUAhHpmcVWm,      & 
& cplUAhSdcSd,cplUAhSecSe,cplUAhSucSu,cplUAhUAhAhAh,cplUAhUAhhhhh,cplUAhUAhHpmcHpm,      & 
& cplUAhUAhSdcSd,cplUAhUAhSecSe,cplUAhUAhSucSu,cplUAhUAhSvcSv,cplUAhUAhcVWmVWm,          & 
...
Else 
Call OneLoopAh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd21L,mHu21L,ms21L,vd,vu,vS,             & 
& TW,TWp,Mhh,Mhh2,MAh,MAh2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,             & 
& MVZ,MVZ2,MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,              & 
& MSv2,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,cplChiChiUAhR,             & 
& cplcFdFdUAhL,cplcFdFdUAhR,cplcFeFeUAhL,cplcFeFeUAhR,cplcFuFuUAhL,cplcFuFuUAhR,         & 
...
End if 
Call OneLoopHpm(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd21L,mHu21L,vd,vu,vS,MHpm,             & 
& MHpm2,MAh,MAh2,MVWm,MVWm2,MChi,MChi2,MCha,MCha2,MFu,MFu2,MFd,MFd2,MFv,MFv2,            & 
& MFe,MFe2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MSu,MSu2,MSd,MSd2,MSv,MSv2,MSe,MSe2,             & 
...
& cplHpmcUHpmVZp,cplSdcUHpmcSu,cplSecUHpmcSv,cplcUHpmVPVWm,cplcUHpmVWmVZ,cplcUHpmVWmVZp, & 
...
Call OneLoopChi(g1,g2,gp,QHd,QHu,Qs,lam,M1,M2,M4,vd,vu,vS,MChi,MChi2,MAh,             & 
& MAh2,MHpm,MHpm2,MCha,MCha2,MVWm,MVWm2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MSd,MSd2,           & 
& MFd,MFd2,MSe,MSe2,MFe,MFe2,MSu,MSu2,MFu,MFu2,MSv,MSv2,MFv,MFv2,cplUChiChiAhL,          & 
& cplUChiChiAhR,cplUChiChacHpmL,cplUChiChacHpmR,cplUChiChacVWmL,cplUChiChacVWmR,         & 
& cplUChiChihhL,cplUChiChihhR,cplUChiChiVZL,cplUChiChiVZR,cplUChiChiVZpL,cplUChiChiVZpR, & 
...
& cplcFvUChiSvR,cplcChaUChiVWmL,cplcChaUChiVWmR,0.1_dp*delta_mass,MChi_1L,               & 
& MChi2_1L,ZN_1L,kont)
...
Call OneLoopVZp(g1,g2,gp,QHd,QHu,Qs,vd,vu,vS,TW,TWp,Mhh,Mhh2,MAh,MAh2,MCha,           & 
& MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,MFv,MFv2,MVZ,MVZ2,MVZp,MVZp2,              & 
& MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,MSv2,cplAhhhVZp,cplcChaChaVZpL,   & 
...
& cplSecSeVZp,cplSucSuVZp,cplSvcSvVZp,cplcVWmVWmVZp,cplAhAhVZpVZp,cplhhhhVZpVZp,         & 
& cplHpmcHpmVZpVZp,cplSdcSdVZpVZp,cplSecSeVZpVZp,cplSucSuVZpVZp,cplSvcSvVZpVZp,          & 
...
& MSv2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,cplAhAhUhh,cplcChaChaUhhL,cplcChaChaUhhR,          & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,cplcgZgZUhh,cplcgZpgZpUhh,     & 
& cplUhhhhhh,cplUhhHpmcHpm,cplUhhSdcSd,cplUhhSecSe,cplUhhSucSu,cplUhhSvcSv,              & 
& cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZpVZp,tadpoles)
...
& MSe2(6),MSu(6),MSu2(6),MSv(3),MSv2(3),MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2

Complex(dp), Intent(in) :: cplAhAhUhh(3,3,3),cplcChaChaUhhL(2,2,3),cplcChaChaUhhR(2,2,3),cplChiChiUhhL(6,6,3),   & 
& cplChiChiUhhR(6,6,3),cplcFdFdUhhL(3,3,3),cplcFdFdUhhR(3,3,3),cplcFeFeUhhL(3,3,3),      & 
& cplcFeFeUhhR(3,3,3),cplcFuFuUhhL(3,3,3),cplcFuFuUhhR(3,3,3),cplcgWmgWmUhh(3),          & 
& cplcgWpCgWpCUhh(3),cplcgZgZUhh(3),cplcgZpgZpUhh(3),cplUhhhhhh(3,3,3),cplUhhHpmcHpm(3,2,2),& 
& cplUhhSdcSd(3,6,6),cplUhhSecSe(3,6,6),cplUhhSucSu(3,6,6),cplUhhSvcSv(3,3,3),           & 
& cplUhhcVWmVWm(3),cplUhhVZVZ(3),cplUhhVZpVZp(3)
...
Subroutine OneLoopSd(g1,g2,gp,Qq,QHd,QHu,Qd,Qs,Yd,Td,lam,mq2,md2,Tdp,vd,              & 
& vu,vS,MSd,MSd2,MAh,MAh2,MFu,MFu2,MCha,MCha2,MFd,MFd2,MChi,MChi2,MGlu,MGlu2,            & 
& Mhh,Mhh2,MSu,MSu2,MHpm,MHpm2,MVZ,MVZ2,MVZp,MVZp2,MVWm,MVWm2,MSe,MSe2,MSv,              & 
& MSv2,cplAhSdcUSd,cplChaFucUSdL,cplChaFucUSdR,cplChiFdcUSdL,cplChiFdcUSdR,              & 
& cplGluFdcUSdL,cplGluFdcUSdR,cplhhSdcUSd,cplHpmSucUSd,cplSdcUSdVG,cplSdcUSdVP,          & 
& cplSdcUSdVZ,cplSdcUSdVZp,cplSucUSdVWm,cplAhAhUSdcUSd,cplhhhhUSdcUSd,cplHpmUSdcHpmcUSd, & 
& cplUSdSdcUSdcSd,cplUSdSecUSdcSe,cplUSdSucUSdcSu,cplUSdSvcUSdcSv,cplUSdcUSdVGVG,        & 
& cplUSdcUSdVPVP,cplUSdcUSdcVWmVWm,cplUSdcUSdVZVZ,cplUSdcUSdVZpVZp,delta,mass,           & 
& mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: MSd(6),MSd2(6),MAh(3),MAh2(3),MFu(3),MFu2(3),MCha(2),MCha2(2),MFd(3),MFd2(3),         & 
& MChi(6),MChi2(6),MGlu,MGlu2,Mhh(3),Mhh2(3),MSu(6),MSu2(6),MHpm(2),MHpm2(2),            & 
& MVZ,MVZ2,MVZp,MVZp2,MVWm,MVWm2,MSe(6),MSe2(6),MSv(3),MSv2(3)

Real(dp), Intent(in) :: g1,g2,gp,Qq,QHd,QHu,Qd,Qs,lam,vd,vu,vS

Complex(dp), Intent(in) :: Yd(3,3),Td(3,3),mq2(3,3),md2(3,3),Tdp(3,3)

Complex(dp), Intent(in) :: cplAhSdcUSd(3,6,6),cplChaFucUSdL(2,3,6),cplChaFucUSdR(2,3,6),cplChiFdcUSdL(6,3,6),    & 
& cplChiFdcUSdR(6,3,6),cplGluFdcUSdL(3,6),cplGluFdcUSdR(3,6),cplhhSdcUSd(3,6,6),         & 
& cplHpmSucUSd(2,6,6),cplSdcUSdVG(6,6),cplSdcUSdVP(6,6),cplSdcUSdVZ(6,6),cplSdcUSdVZp(6,6),& 
& cplSucUSdVWm(6,6),cplAhAhUSdcUSd(3,3,6,6),cplhhhhUSdcUSd(3,3,6,6),cplHpmUSdcHpmcUSd(2,6,2,6),& 
& cplUSdSdcUSdcSd(6,6,6,6),cplUSdSecUSdcSe(6,6,6,6),cplUSdSucUSdcSu(6,6,6,6),            & 
...
Subroutine OneLoopSv(g1,g2,gp,Ql,QHd,QHu,Qs,ml122,ml12l32,ml32,vd,vu,vS,              & 
& MFv,MFv2,MChi,MChi2,MCha,MCha2,MFe,MFe2,MSv,MSv2,Mhh,Mhh2,MHpm,MHpm2,MSe,              & 
& MSe2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MAh,MAh2,MSd,MSd2,MSu,MSu2,cplChiFvcUSvL,          & 
& cplChiFvcUSvR,cplcChaFecUSvL,cplcChaFecUSvR,cplhhSvcUSv,cplSecHpmcUSv,cplSecUSvcVWm,   & 
& cplSvcUSvVZ,cplSvcUSvVZp,cplAhAhUSvcUSv,cplhhhhUSvcUSv,cplHpmUSvcHpmcUSv,              & 
& cplSdUSvcSdcUSv,cplSeUSvcSecUSv,cplSuUSvcSucUSv,cplUSvSvcUSvcSv,cplUSvcUSvcVWmVWm,     & 
& cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,delta,mass,mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: MFv(3),MFv2(3),MChi(6),MChi2(6),MCha(2),MCha2(2),MFe(3),MFe2(3),MSv(3),               & 
& MSv2(3),Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MSe(6),MSe2(6),MVWm,MVWm2,MVZ,MVZ2,            & 
& MVZp,MVZp2,MAh(3),MAh2(3),MSd(6),MSd2(6),MSu(6),MSu2(6)

Real(dp), Intent(in) :: g1,g2,gp,Ql,QHd,QHu,Qs,ml12l32(2),ml32,vd,vu,vS

Complex(dp), Intent(in) :: ml122(2,2)

Complex(dp), Intent(in) :: cplChiFvcUSvL(6,3,3),cplChiFvcUSvR(6,3,3),cplcChaFecUSvL(2,3,3),cplcChaFecUSvR(2,3,3),& 
& cplhhSvcUSv(3,3,3),cplSecHpmcUSv(6,2,3),cplSecUSvcVWm(6,3),cplSvcUSvVZ(3,3),           & 
& cplSvcUSvVZp(3,3),cplAhAhUSvcUSv(3,3,3,3),cplhhhhUSvcUSv(3,3,3,3),cplHpmUSvcHpmcUSv(2,3,2,3),& 
& cplSdUSvcSdcUSv(6,3,6,3),cplSeUSvcSecUSv(6,3,6,3),cplSuUSvcSucUSv(6,3,6,3),            & 
& cplUSvSvcUSvcSv(3,3,3,3),cplUSvcUSvcVWmVWm(3,3),cplUSvcUSvVZVZ(3,3),cplUSvcUSvVZpVZp(3,3)
...
mat2a(1,1) = mat2a(1,1)+ml122(1,1)
mat2a(1,2) = 0._dp 
mat2a(1,2) = mat2a(1,2)+ml122(1,2)
mat2a(1,3) = 0._dp 
mat2a(1,3) = mat2a(1,3)+ml12l32(1)
mat2a(2,2) = 0._dp 
...
mat2a(2,2) = mat2a(2,2)+ml122(2,2)
mat2a(2,3) = 0._dp 
mat2a(2,3) = mat2a(2,3)+ml12l32(2)
mat2a(3,3) = 0._dp 
mat2a(3,3) = mat2a(3,3)+ml32
...
& cplUSvcUSvcVWmVWm,cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,kont,PiSf(1,:,:))
...
& cplSecHpmcUSv,cplSecUSvcVWm,cplSvcUSvVZ,cplSvcUSvVZp,cplAhAhUSvcUSv,cplhhhhUSvcUSv,    & 
& cplHpmUSvcHpmcUSv,cplSdUSvcSdcUSv,cplSeUSvcSecUSv,cplSuUSvcSucUSv,cplUSvSvcUSvcSv,     & 
& cplUSvcUSvcVWmVWm,cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,kont,PiSf(i1,:,:))
...
Subroutine Pi1LoopSv(p2,MFv,MFv2,MChi,MChi2,MCha,MCha2,MFe,MFe2,MSv,MSv2,             & 
& Mhh,Mhh2,MHpm,MHpm2,MSe,MSe2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MAh,MAh2,MSd,              & 
& MSd2,MSu,MSu2,cplChiFvcUSvL,cplChiFvcUSvR,cplcChaFecUSvL,cplcChaFecUSvR,               & 
...
Subroutine OneLoopFv(MSv,MSv2,MChi,MChi2,MHpm,MHpm2,MFe,MFe2,MVWm,MVWm2,              & 
& MVZ,MVZ2,MFv,MFv2,MVZp,MVZp2,MCha,MCha2,MSe,MSe2,cplcUFvChiSvL,cplcUFvChiSvR,          & 
& cplcUFvFecHpmL,cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvVZL,              & 
& cplcUFvFvVZR,cplcUFvFvVZpL,cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,delta,          & 
& MFv_1L,MFv2_1L,ZVL_1L,ierr)

Implicit None 
Real(dp), Intent(in) :: MSv(3),MSv2(3),MChi(6),MChi2(6),MHpm(2),MHpm2(2),MFe(3),MFe2(3),MVWm,MVWm2,           & 
& MVZ,MVZ2,MFv(3),MFv2(3),MVZp,MVZp2,MCha(2),MCha2(2),MSe(6),MSe2(6)

Complex(dp), Intent(in) :: cplcUFvChiSvL(3,6,3),cplcUFvChiSvR(3,6,3),cplcUFvFecHpmL(3,3,2),cplcUFvFecHpmR(3,3,2),& 
...
Complex(dp), Intent(out) ::  ZVL_1L(3,3) 
Real(dp) :: MFv_t(3),MFv2_t(3) 
Complex(dp) ::  ZVL_t(3,3) 
...
& MVZ,MVZ2,MFv,MFv2,MVZp,MVZp2,MCha,MCha2,MSe,MSe2,cplcUFvChiSvL,cplcUFvChiSvR,          & 
& cplcUFvFecHpmL,cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvVZL,              & 
& cplcUFvFvVZR,cplcUFvFvVZpL,cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,sigL,           & 
& sigR,sigSL,sigSR)
...
Call EigenSystem(Real(mat1,dp),Eig,ZVLa,ierr,test) 
 
   Do i1=1,3
   If (Eig(i1).Lt.0._dp) Then 
    MFv_t(i1) = - Eig(i1) 
    ZVL_1L(i1,:) = (0._dp,1._dp)*ZVLa(i1,:) 
   Else 
    MFv_t(i1) = Eig(i1) 
    ZVL_1L(i1,:) = ZVLa(i1,:)
    End If 
...
Call Eigensystem(mat2, Eig, ZVL_1L, ierr, test) 
mat2 = Matmul( Conjg(ZVL_1L), Matmul( mat1, Transpose( Conjg(ZVL_1L)))) 
Do i1=1,3
If (Abs(mat2(i1,i1)).gt.0._dp) Then 
  phaseM = Sqrt( mat2(i1,i1) / Abs(mat2(i1,i1))) 
  ZVL_1L(i1,:)= phaseM * ZVL_1L(i1,:) 
End if 
ZVLOS_0 = ZVL_1L 
 End Do 
End If 
...
& MVWm2,MVZ,MVZ2,MFv,MFv2,MVZp,MVZp2,MCha,MCha2,MSe,MSe2,cplcUFvChiSvL,cplcUFvChiSvR,    & 
& cplcUFvFecHpmL,cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvVZL,              & 
& cplcUFvFvVZR,cplcUFvFvVZpL,cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,sigL,           & 
& sigR,sigSL,sigSR)
...
Subroutine OneLoopSu(g1,g2,gp,Qq,QHd,QHu,Qu,Qs,lam,Yu,Tu,mq2,mu2,vd,vu,               & 
& vS,MSu,MSu2,MAh,MAh2,MFu,MFu2,MChi,MChi2,MCha,MCha2,MFd,MFd2,MGlu,MGlu2,               & 
& Mhh,Mhh2,MHpm,MHpm2,MSd,MSd2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSe,MSe2,MSv,              & 
& MSv2,cplAhSucUSu,cplChiFucUSuL,cplChiFucUSuR,cplcChaFdcUSuL,cplcChaFdcUSuR,            & 
...
& cplSdUSucSdcUSu,cplSeUSucSecUSu,cplUSuSucUSucSu,cplUSuSvcUSucSv,cplUSucUSuVGVG,        & 
& cplUSucUSuVPVP,cplUSucUSucVWmVWm,cplUSucUSuVZVZ,cplUSucUSuVZpVZp,delta,mass,           & 
& mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: MSu(6),MSu2(6),MAh(3),MAh2(3),MFu(3),MFu2(3),MChi(6),MChi2(6),MCha(2),MCha2(2),       & 
& MFd(3),MFd2(3),MGlu,MGlu2,Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MSd(6),MSd2(6),              & 
& MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSe(6),MSe2(6),MSv(3),MSv2(3)

Real(dp), Intent(in) :: g1,g2,gp,Qq,QHd,QHu,Qu,Qs,lam,vd,vu,vS

Complex(dp), Intent(in) :: Yu(3,3),Tu(3,3),mq2(3,3),mu2(3,3)

Complex(dp), Intent(in) :: cplAhSucUSu(3,6,6),cplChiFucUSuL(6,3,6),cplChiFucUSuR(6,3,6),cplcChaFdcUSuL(2,3,6),   & 
...
& cplSucUSuVZ(6,6),cplSucUSuVZp(6,6),cplAhAhUSucUSu(3,3,6,6),cplhhhhUSucUSu(3,3,6,6),    & 
& cplHpmUSucHpmcUSu(2,6,2,6),cplSdUSucSdcUSu(6,6,6,6),cplSeUSucSecUSu(6,6,6,6),          & 
& cplUSuSucUSucSu(6,6,6,6),cplUSuSvcUSucSv(6,3,6,3),cplUSucUSuVGVG(6,6),cplUSucUSuVPVP(6,6),& 
...
Subroutine OneLoopSe(g1,g2,gp,Ql,QHd,QHu,Qe12,Qe3,Qs,y12,Ty12,y3,Ty3,lam,             & 
& ml122,ml12l32,ml32,me122,me32,Tep,vd,vu,vS,MSe,MSe2,MAh,MAh2,MFv,MFv2,MCha,            & 
& MCha2,MFe,MFe2,MChi,MChi2,Mhh,Mhh2,MSv,MSv2,MHpm,MHpm2,MVZ,MVZ2,MVZp,MVZp2,            & 
& MVWm,MVWm2,MSd,MSd2,MSu,MSu2,cplAhSecUSe,cplChaFvcUSeL,cplChaFvcUSeR,cplChiFecUSeL,    & 
& cplChiFecUSeR,cplhhSecUSe,cplHpmSvcUSe,cplSecUSeVP,cplSecUSeVZ,cplSecUSeVZp,           & 
& cplSvcUSeVWm,cplAhAhUSecUSe,cplhhhhUSecUSe,cplHpmUSecHpmcUSe,cplSdUSecSdcUSe,          & 
& cplUSeSecUSecSe,cplUSeSucUSecSu,cplUSeSvcUSecSv,cplUSecUSeVPVP,cplUSecUSecVWmVWm,      & 
& cplUSecUSeVZVZ,cplUSecUSeVZpVZp,delta,mass,mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: MSe(6),MSe2(6),MAh(3),MAh2(3),MFv(3),MFv2(3),MCha(2),MCha2(2),MFe(3),MFe2(3),         & 
& MChi(6),MChi2(6),Mhh(3),Mhh2(3),MSv(3),MSv2(3),MHpm(2),MHpm2(2),MVZ,MVZ2,              & 
& MVZp,MVZp2,MVWm,MVWm2,MSd(6),MSd2(6),MSu(6),MSu2(6)

Real(dp), Intent(in) :: g1,g2,gp,Ql,QHd,QHu,Qe12,Qe3,Qs,lam,ml12l32(2),ml32,me32,vd,vu,vS

Complex(dp), Intent(in) :: y12(2,2),Ty12(2,2),y3,Ty3,ml122(2,2),me122(2,2),Tep(2,2)

Complex(dp), Intent(in) :: cplAhSecUSe(3,6,6),cplChaFvcUSeL(2,3,6),cplChaFvcUSeR(2,3,6),cplChiFecUSeL(6,3,6),    & 
& cplChiFecUSeR(6,3,6),cplhhSecUSe(3,6,6),cplHpmSvcUSe(2,3,6),cplSecUSeVP(6,6),          & 
& cplSecUSeVZ(6,6),cplSecUSeVZp(6,6),cplSvcUSeVWm(3,6),cplAhAhUSecUSe(3,3,6,6),          & 
& cplhhhhUSecUSe(3,3,6,6),cplHpmUSecHpmcUSe(2,6,2,6),cplSdUSecSdcUSe(6,6,6,6),           & 
...
mat2a(1,1) = mat2a(1,1)+ml122(1,1)
Do j1 = 1,2
mat2a(1,1) = mat2a(1,1)+(vd**2*Conjg(y12(j1,1))*y12(j1,1))/2._dp
End Do 
mat2a(1,2) = 0._dp 
mat2a(1,2) = mat2a(1,2)+ml122(1,2)
Do j1 = 1,2
mat2a(1,2) = mat2a(1,2)+(vd**2*Conjg(y12(j1,1))*y12(j1,2))/2._dp
End Do 
mat2a(1,3) = 0._dp 
mat2a(1,3) = mat2a(1,3)+ml12l32(1)
mat2a(1,4) = 0._dp 
mat2a(1,4) = mat2a(1,4)-((vu*Conjg(Tep(1,1)))/sqrt(2._dp))
mat2a(1,4) = mat2a(1,4)-(vS*vu*lam*Conjg(y12(1,1)))/2._dp
mat2a(1,4) = mat2a(1,4)+(vd*Conjg(Ty12(1,1)))/sqrt(2._dp)
mat2a(1,5) = 0._dp 
mat2a(1,5) = mat2a(1,5)-((vu*Conjg(Tep(2,1)))/sqrt(2._dp))
mat2a(1,5) = mat2a(1,5)-(vS*vu*lam*Conjg(y12(2,1)))/2._dp
mat2a(1,5) = mat2a(1,5)+(vd*Conjg(Ty12(2,1)))/sqrt(2._dp)
mat2a(1,6) = 0._dp 
mat2a(2,2) = 0._dp 
mat2a(2,2) = mat2a(2,2)+(g1**2*vd**2)/8._dp
mat2a(2,2) = mat2a(2,2)-(g2**2*vd**2)/8._dp
...
mat2a(2,2) = mat2a(2,2)+ml122(2,2)
Do j1 = 1,2
mat2a(2,2) = mat2a(2,2)+(vd**2*Conjg(y12(j1,2))*y12(j1,2))/2._dp
End Do 
mat2a(2,3) = 0._dp 
mat2a(2,3) = mat2a(2,3)+ml12l32(2)
mat2a(2,4) = 0._dp 
mat2a(2,4) = mat2a(2,4)-((vu*Conjg(Tep(1,2)))/sqrt(2._dp))
mat2a(2,4) = mat2a(2,4)-(vS*vu*lam*Conjg(y12(1,2)))/2._dp
mat2a(2,4) = mat2a(2,4)+(vd*Conjg(Ty12(1,2)))/sqrt(2._dp)
mat2a(2,5) = 0._dp 
mat2a(2,5) = mat2a(2,5)-((vu*Conjg(Tep(2,2)))/sqrt(2._dp))
mat2a(2,5) = mat2a(2,5)-(vS*vu*lam*Conjg(y12(2,2)))/2._dp
mat2a(2,5) = mat2a(2,5)+(vd*Conjg(Ty12(2,2)))/sqrt(2._dp)
mat2a(2,6) = 0._dp 
mat2a(3,3) = 0._dp 
mat2a(3,3) = mat2a(3,3)+ml32
mat2a(3,3) = mat2a(3,3)+(g1**2*vd**2)/8._dp
...
mat2a(3,3) = mat2a(3,3)+(g2**2*vu**2)/8._dp
mat2a(3,3) = mat2a(3,3)+(gp**2*QHu*Ql*vu**2)/2._dp
mat2a(3,3) = mat2a(3,3)+(vd**2*y3*Conjg(y3))/2._dp
mat2a(3,4) = 0._dp 
mat2a(3,5) = 0._dp 
mat2a(3,6) = 0._dp 
mat2a(3,6) = mat2a(3,6)-(vS*vu*lam*Conjg(y3))/2._dp
mat2a(3,6) = mat2a(3,6)+(vd*Conjg(Ty3))/sqrt(2._dp)
mat2a(4,4) = 0._dp 
mat2a(4,4) = mat2a(4,4)-(g1**2*vd**2)/4._dp
mat2a(4,4) = mat2a(4,4)+(gp**2*Qe12*QHd*vd**2)/2._dp
mat2a(4,4) = mat2a(4,4)+(gp**2*Qe12*Qs*vS**2)/2._dp
mat2a(4,4) = mat2a(4,4)+(g1**2*vu**2)/4._dp
mat2a(4,4) = mat2a(4,4)+(gp**2*Qe12*QHu*vu**2)/2._dp
mat2a(4,4) = mat2a(4,4)+me122(1,1)
Do j1 = 1,2
mat2a(4,4) = mat2a(4,4)+(vd**2*Conjg(y12(1,j1))*y12(1,j1))/2._dp
End Do 
mat2a(4,5) = 0._dp 
mat2a(4,5) = mat2a(4,5)+me122(1,2)
Do j1 = 1,2
mat2a(4,5) = mat2a(4,5)+(vd**2*Conjg(y12(2,j1))*y12(1,j1))/2._dp
End Do 
mat2a(4,6) = 0._dp 
mat2a(5,5) = 0._dp 
mat2a(5,5) = mat2a(5,5)-(g1**2*vd**2)/4._dp
mat2a(5,5) = mat2a(5,5)+(gp**2*Qe12*QHd*vd**2)/2._dp
mat2a(5,5) = mat2a(5,5)+(gp**2*Qe12*Qs*vS**2)/2._dp
mat2a(5,5) = mat2a(5,5)+(g1**2*vu**2)/4._dp
mat2a(5,5) = mat2a(5,5)+(gp**2*Qe12*QHu*vu**2)/2._dp
mat2a(5,5) = mat2a(5,5)+me122(2,2)
Do j1 = 1,2
mat2a(5,5) = mat2a(5,5)+(vd**2*Conjg(y12(2,j1))*y12(2,j1))/2._dp
End Do 
mat2a(5,6) = 0._dp 
mat2a(6,6) = 0._dp 
mat2a(6,6) = mat2a(6,6)+me32
mat2a(6,6) = mat2a(6,6)-(g1**2*vd**2)/4._dp
mat2a(6,6) = mat2a(6,6)+(gp**2*Qe3*QHd*vd**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(gp**2*Qe3*Qs*vS**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(g1**2*vu**2)/4._dp
mat2a(6,6) = mat2a(6,6)+(gp**2*Qe3*QHu*vu**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(vd**2*y3*Conjg(y3))/2._dp
...
Subroutine OneLoophh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,ms2,vd,vu,vS,             & 
& MAh,MAh2,MVZ,MVZ2,MVZp,MVZp2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,              & 
& MFu2,Mhh,Mhh2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,MSv2,               & 
& cplAhAhUhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,cplChiChiUhhL,         & 
& cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,cplcFuFuUhhL,        & 
...
Implicit None 
Real(dp), Intent(in) :: MAh(3),MAh2(3),MVZ,MVZ2,MVZp,MVZp2,MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),          & 
& MFd2(3),MFe(3),MFe2(3),MFu(3),MFu2(3),Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MVWm,            & 
& MVWm2,MSd(6),MSd2(6),MSe(6),MSe2(6),MSu(6),MSu2(6),MSv(3),MSv2(3)

Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,ms2,vd,vu,vS

Complex(dp), Intent(in) :: cplAhAhUhh(3,3,3),cplAhUhhVZ(3,3),cplAhUhhVZp(3,3),cplcChaChaUhhL(2,2,3),             & 
...
mat2a(1,1) = mat2a(1,1)+(vS**2*lam**2)/2._dp
mat2a(1,1) = mat2a(1,1)+(vu**2*lam**2)/2._dp
mat2a(1,2) = 0._dp 
mat2a(1,2) = mat2a(1,2)-(g1**2*vd*vu)/4._dp
mat2a(1,2) = mat2a(1,2)-(g2**2*vd*vu)/4._dp
mat2a(1,2) = mat2a(1,2)+gp**2*QHd*QHu*vd*vu
mat2a(1,2) = mat2a(1,2)+vd*vu*lam**2
mat2a(1,2) = mat2a(1,2)-((vS*Tlam)/sqrt(2._dp))
mat2a(1,3) = 0._dp 
mat2a(1,3) = mat2a(1,3)+gp**2*QHd*Qs*vd*vS
mat2a(1,3) = mat2a(1,3)+vd*vS*lam**2
mat2a(1,3) = mat2a(1,3)-((vu*Tlam)/sqrt(2._dp))
mat2a(2,2) = 0._dp 
...
mat2a(2,2) = mat2a(2,2)+(vd**2*lam**2)/2._dp
mat2a(2,2) = mat2a(2,2)+(vS**2*lam**2)/2._dp
mat2a(2,3) = 0._dp 
mat2a(2,3) = mat2a(2,3)+gp**2*QHu*Qs*vS*vu
mat2a(2,3) = mat2a(2,3)+vS*vu*lam**2
mat2a(2,3) = mat2a(2,3)-((vd*Tlam)/sqrt(2._dp))
mat2a(3,3) = 0._dp 
...
mat2a(3,3) = mat2a(3,3)+(vd**2*lam**2)/2._dp
mat2a(3,3) = mat2a(3,3)+(vu**2*lam**2)/2._dp
...
& MSu,MSu2,MSv,MSv2,cplAhAhUhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,     & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,cplcgZgZUhh,cplcgZpgZUhh,      & 
& cplcgZgZpUhh,cplcgZpgZpUhh,cplUhhhhhh,cplUhhHpmcHpm,cplUhhHpmcVWm,cplUhhSdcSd,         & 
& cplUhhSecSe,cplUhhSucSu,cplUhhSvcSv,cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZVZp,              & 
...
Subroutine OneLoopAh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,ms2,vd,vu,vS,             & 
& TW,TWp,Mhh,Mhh2,MAh,MAh2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,             & 
& MVZ,MVZ2,MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,              & 
& MSv2,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,cplChiChiUAhR,             & 
& cplcFdFdUAhL,cplcFdFdUAhR,cplcFeFeUAhL,cplcFeFeUAhR,cplcFuFuUAhL,cplcFuFuUAhR,         & 
...
& cplUAhSdcSd,cplUAhSecSe,cplUAhSucSu,cplUAhUAhAhAh,cplUAhUAhhhhh,cplUAhUAhHpmcHpm,      & 
& cplUAhUAhSdcSd,cplUAhUAhSecSe,cplUAhUAhSucSu,cplUAhUAhSvcSv,cplUAhUAhcVWmVWm,          & 
& cplUAhUAhVZVZ,cplUAhUAhVZpVZp,delta,mass,mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: Mhh(3),Mhh2(3),MAh(3),MAh2(3),MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),               & 
& MFd2(3),MFe(3),MFe2(3),MFu(3),MFu2(3),MVZ,MVZ2,MVZp,MVZp2,MHpm(2),MHpm2(2),            & 
& MVWm,MVWm2,MSd(6),MSd2(6),MSe(6),MSe2(6),MSu(6),MSu2(6),MSv(3),MSv2(3)

Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,ms2,vd,vu,vS,TW,TWp

Complex(dp), Intent(in) :: cplUAhAhhh(3,3,3),cplcChaChaUAhL(2,2,3),cplcChaChaUAhR(2,2,3),cplChiChiUAhL(6,6,3),   & 
& cplChiChiUAhR(6,6,3),cplcFdFdUAhL(3,3,3),cplcFdFdUAhR(3,3,3),cplcFeFeUAhL(3,3,3),      & 
& cplcFeFeUAhR(3,3,3),cplcFuFuUAhL(3,3,3),cplcFuFuUAhR(3,3,3),cplcgWmgWmUAh(3),          & 
& cplcgWpCgWpCUAh(3),cplUAhhhVZ(3,3),cplUAhhhVZp(3,3),cplUAhHpmcHpm(3,2,2),              & 
...
& cplUAhUAhSvcSv(3,3,3,3),cplUAhUAhcVWmVWm(3,3),cplUAhUAhVZVZ(3,3),cplUAhUAhVZpVZp(3,3)
...
& MSu,MSu2,MSv,MSv2,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,              & 
& cplChiChiUAhR,cplcFdFdUAhL,cplcFdFdUAhR,cplcFeFeUAhL,cplcFeFeUAhR,cplcFuFuUAhL,        & 
& cplcFuFuUAhR,cplcgWmgWmUAh,cplcgWpCgWpCUAh,cplUAhhhVZ,cplUAhhhVZp,cplUAhHpmcHpm,       & 
...
& cplUAhUAhcVWmVWm,cplUAhUAhVZVZ,cplUAhUAhVZpVZp,kont,PiSf(1,:,:))
...
& cplUAhHpmcVWm,cplUAhSdcSd,cplUAhSecSe,cplUAhSucSu,cplUAhUAhAhAh,cplUAhUAhhhhh,         & 
& cplUAhUAhHpmcHpm,cplUAhUAhSdcSd,cplUAhUAhSecSe,cplUAhUAhSucSu,cplUAhUAhSvcSv,          & 
& cplUAhUAhcVWmVWm,cplUAhUAhVZVZ,cplUAhUAhVZpVZp,kont,PiSf(i1,:,:))
...
& MFe,MFe2,MFu,MFu2,MVZ,MVZ2,MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,              & 
& MSe2,MSu,MSu2,MSv,MSv2,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,         & 
& cplChiChiUAhR,cplcFdFdUAhL,cplcFdFdUAhR,cplcFeFeUAhL,cplcFeFeUAhR,cplcFuFuUAhL,        & 
& cplcFuFuUAhR,cplcgWmgWmUAh,cplcgWpCgWpCUAh,cplUAhhhVZ,cplUAhhhVZp,cplUAhHpmcHpm,       & 
& cplUAhHpmcVWm,cplUAhSdcSd,cplUAhSecSe,cplUAhSucSu,cplUAhUAhAhAh,cplUAhUAhhhhh,         & 
& cplUAhUAhHpmcHpm,cplUAhUAhSdcSd,cplUAhUAhSecSe,cplUAhUAhSucSu,cplUAhUAhSvcSv,          & 
...
Subroutine OneLoopHpm(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,vd,vu,vS,MHpm,           & 
& MHpm2,MAh,MAh2,MVWm,MVWm2,MChi,MChi2,MCha,MCha2,MFu,MFu2,MFd,MFd2,MFv,MFv2,            & 
& MFe,MFe2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MSu,MSu2,MSd,MSd2,MSv,MSv2,MSe,MSe2,             & 
& cplAhHpmcUHpm,cplAhcUHpmVWm,cplChiChacUHpmL,cplChiChacUHpmR,cplcFuFdcUHpmL,            & 
& cplcFuFdcUHpmR,cplcFvFecUHpmL,cplcFvFecUHpmR,cplcgZgWmcUHpm,cplcgWmgZUHpm,             & 
...
Implicit None 
Real(dp), Intent(in) :: MHpm(2),MHpm2(2),MAh(3),MAh2(3),MVWm,MVWm2,MChi(6),MChi2(6),MCha(2),MCha2(2),         & 
& MFu(3),MFu2(3),MFd(3),MFd2(3),MFv(3),MFv2(3),MFe(3),MFe2(3),Mhh(3),Mhh2(3),            & 
& MVZ,MVZ2,MVZp,MVZp2,MSu(6),MSu2(6),MSd(6),MSd2(6),MSv(3),MSv2(3),MSe(6),MSe2(6)

Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd2,mHu2,vd,vu,vS

Complex(dp), Intent(in) :: cplAhHpmcUHpm(3,2,2),cplAhcUHpmVWm(3,2),cplChiChacUHpmL(6,2,2),cplChiChacUHpmR(6,2,2),& 
...
& cplHpmcUHpmVZp(2,2),cplSdcUHpmcSu(6,2,6),cplSecUHpmcSv(6,2,3),cplcUHpmVPVWm(2),        & 
& cplcUHpmVWmVZ(2),cplcUHpmVWmVZp(2),cplAhAhUHpmcUHpm(3,3,2,2),cplhhhhUHpmcUHpm(3,3,2,2),& 
& cplUHpmHpmcUHpmcHpm(2,2,2,2),cplUHpmSdcUHpmcSd(2,6,2,6),cplUHpmSecUHpmcSe(2,6,2,6),    & 
& cplUHpmSucUHpmcSu(2,6,2,6),cplUHpmSvcUHpmcSv(2,3,2,3),cplUHpmcUHpmVPVP(2,2),           & 
...
mat2a(1,2) = 0._dp 
mat2a(1,2) = mat2a(1,2)+(g2**2*vd*vu)/4._dp
mat2a(1,2) = mat2a(1,2)-(vd*vu*lam**2)/2._dp
...
Subroutine OneLoopChi(g1,g2,gp,QHd,QHu,Qs,lam,M1,M2,M4,vd,vu,vS,MChi,MChi2,           & 
& MAh,MAh2,MHpm,MHpm2,MCha,MCha2,MVWm,MVWm2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MSd,            & 
& MSd2,MFd,MFd2,MSe,MSe2,MFe,MFe2,MSu,MSu2,MFu,MFu2,MSv,MSv2,MFv,MFv2,cplUChiChiAhL,     & 
& cplUChiChiAhR,cplUChiChacHpmL,cplUChiChacHpmR,cplUChiChacVWmL,cplUChiChacVWmR,         & 
& cplUChiChihhL,cplUChiChihhR,cplUChiChiVZL,cplUChiChiVZR,cplUChiChiVZpL,cplUChiChiVZpR, & 
& cplUChiFdcSdL,cplUChiFdcSdR,cplUChiFecSeL,cplUChiFecSeR,cplUChiFucSuL,cplUChiFucSuR,   & 
& cplUChiFvcSvL,cplUChiFvcSvR,cplcChaUChiHpmL,cplcChaUChiHpmR,cplcFdUChiSdL,             & 
& cplcFdUChiSdR,cplcFeUChiSeL,cplcFeUChiSeR,cplcFuUChiSuL,cplcFuUChiSuR,cplcFvUChiSvL,   & 
& cplcFvUChiSvR,cplcChaUChiVWmL,cplcChaUChiVWmR,delta,MChi_1L,MChi2_1L,ZN_1L,ierr)

Implicit None 
Real(dp), Intent(in) :: MChi(6),MChi2(6),MAh(3),MAh2(3),MHpm(2),MHpm2(2),MCha(2),MCha2(2),MVWm,               & 
& MVWm2,Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MSd(6),MSd2(6),MFd(3),MFd2(3),MSe(6),         & 
& MSe2(6),MFe(3),MFe2(3),MSu(6),MSu2(6),MFu(3),MFu2(3),MSv(3),MSv2(3),MFv(3),MFv2(3)

Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,lam,vd,vu,vS

Complex(dp), Intent(in) :: M1,M2,M4

Complex(dp), Intent(in) :: cplUChiChiAhL(6,6,3),cplUChiChiAhR(6,6,3),cplUChiChacHpmL(6,2,2),cplUChiChacHpmR(6,2,2),& 
& cplUChiChacVWmL(6,2),cplUChiChacVWmR(6,2),cplUChiChihhL(6,6,3),cplUChiChihhR(6,6,3),   & 
& cplUChiChiVZL(6,6),cplUChiChiVZR(6,6),cplUChiChiVZpL(6,6),cplUChiChiVZpR(6,6),         & 
& cplUChiFdcSdL(6,3,6),cplUChiFdcSdR(6,3,6),cplUChiFecSeL(6,3,6),cplUChiFecSeR(6,3,6),   & 
& cplUChiFucSuL(6,3,6),cplUChiFucSuR(6,3,6),cplUChiFvcSvL(6,3,3),cplUChiFvcSvR(6,3,3),   & 
& cplcChaUChiHpmL(2,6,2),cplcChaUChiHpmR(2,6,2),cplcFdUChiSdL(3,6,6),cplcFdUChiSdR(3,6,6),& 
& cplcFeUChiSeL(3,6,6),cplcFeUChiSeR(3,6,6),cplcFuUChiSuL(3,6,6),cplcFuUChiSuR(3,6,6),   & 
& cplcFvUChiSvL(3,6,3),cplcFvUChiSvR(3,6,3),cplcChaUChiVWmL(2,6),cplcChaUChiVWmR(2,6)
...
Real(dp), Intent(out) :: MChi_1L(6),MChi2_1L(6) 
Complex(dp), Intent(out) ::  ZN_1L(6,6) 
Real(dp) :: MChi_t(6),MChi2_t(6) 
Complex(dp) ::  ZN_t(6,6) 
...
Implicit None 
Real(dp), Intent(in) :: MCha(2),MCha2(2),MAh(3),MAh2(3),Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MHpm(2),           & 
& MHpm2(2),MChi(6),MChi2(6),MVWm,MVWm2,MSu(6),MSu2(6),MFd(3),MFd2(3),MSv(3),             & 
& MSv2(3),MFe(3),MFe2(3),MFu(3),MFu2(3),MSd(6),MSd2(6),MFv(3),MFv2(3),MSe(6),MSe2(6)
...
& cplcUChaChaVZpL(2,2),cplcUChaChaVZpR(2,2),cplcUChaChiHpmL(2,6,2),cplcUChaChiHpmR(2,6,2),& 
& cplcUChaChiVWmL(2,6),cplcUChaChiVWmR(2,6),cplcUChaFdcSuL(2,3,6),cplcUChaFdcSuR(2,3,6), & 
& cplcUChaFecSvL(2,3,3),cplcUChaFecSvR(2,3,3),cplcUChacFuSdL(2,3,6),cplcUChacFuSdR(2,3,6),& 
...
Subroutine OneLoopFe(y12,y3,vd,MFe,MFe2,MAh,MAh2,MSv,MSv2,MCha,MCha2,MSe,             & 
...
& MChi(6),MChi2(6),Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MHpm(2),MHpm2(2),MFv(3),           & 
& MFv2(3),MVWm,MVWm2

Real(dp), Intent(in) :: vd

Complex(dp), Intent(in) :: y12(2,2),y3
...
mat1a(1,1) = 0._dp 
mat1a(1,1) = mat1a(1,1)+(vd*y12(1,1))/sqrt(2._dp)
mat1a(1,2) = 0._dp 
mat1a(1,2) = mat1a(1,2)+(vd*y12(2,1))/sqrt(2._dp)
mat1a(1,3) = 0._dp 
mat1a(2,1) = 0._dp 
mat1a(2,1) = mat1a(2,1)+(vd*y12(1,2))/sqrt(2._dp)
mat1a(2,2) = 0._dp 
mat1a(2,2) = mat1a(2,2)+(vd*y12(2,2))/sqrt(2._dp)
mat1a(2,3) = 0._dp 
mat1a(3,1) = 0._dp 
mat1a(3,2) = 0._dp 
mat1a(3,3) = 0._dp 
mat1a(3,3) = mat1a(3,3)+(vd*y3)/sqrt(2._dp)
...
& MChi(6),MChi2(6),Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MHpm(2),MHpm2(2),MFu(3),           & 
...
& cplcUFdChiSdL(3,6,6),cplcUFdChiSdR(3,6,6),cplcUFdFdhhL(3,3,3),cplcUFdFdhhR(3,3,3),     & 
& cplcUFdFdVGL(3,3),cplcUFdFdVGR(3,3),cplcUFdFdVPL(3,3),cplcUFdFdVPR(3,3),               & 
...
Implicit None 
Real(dp), Intent(in) :: MFu(3),MFu2(3),MAh(3),MAh2(3),MSu(6),MSu2(6),MChi(6),MChi2(6),MHpm(2),MHpm2(2),       & 
& MFd(3),MFd2(3),MVWm,MVWm2,Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MGlu,MGlu2,               & 
...
Complex(dp), Intent(in) :: cplcUFuFuAhL(3,3,3),cplcUFuFuAhR(3,3,3),cplcUFuChiSuL(3,6,6),cplcUFuChiSuR(3,6,6),    & 
...
Subroutine OneLoopVZ(g1,g2,gp,QHd,QHu,Qs,vd,vu,vS,TW,TWp,Mhh,Mhh2,MAh,MAh2,           & 
& MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,MFv,MFv2,MVZ,MVZ2,MVZp,               & 
& MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,MSv2,cplAhhhVZ,             & 
...
& cplcgWmgWmVZ,cplcgWpCgWpCVZ,cplhhVZVZ,cplhhVZVZp,cplHpmcHpmVZ,cplHpmcVWmVZ,            & 
...
Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,vd,vu,vS,TW,TWp

Complex(dp), Intent(in) :: cplAhhhVZ(3,3),cplcChaChaVZL(2,2),cplcChaChaVZR(2,2),cplChiChiVZL(6,6),               & 
& cplChiChiVZR(6,6),cplcFdFdVZL(3,3),cplcFdFdVZR(3,3),cplcFeFeVZL(3,3),cplcFeFeVZR(3,3), & 
& cplcFuFuVZL(3,3),cplcFuFuVZR(3,3),cplcFvFvVZL(3,3),cplcFvFvVZR(3,3),cplcgWmgWmVZ,      & 
& cplcgWpCgWpCVZ,cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmVZ(2,2),cplHpmcVWmVZ(2),           & 
& cplSdcSdVZ(6,6),cplSecSeVZ(6,6),cplSucSuVZ(6,6),cplSvcSvVZ(3,3),cplcVWmVWmVZ,          & 
& cplAhAhVZVZ(3,3),cplhhhhVZVZ(3,3),cplHpmcHpmVZVZ(2,2),cplSdcSdVZVZ(6,6),               & 
& cplSecSeVZVZ(6,6),cplSucSuVZVZ(6,6),cplSvcSvVZVZ(3,3),cplcVWmVWmVZVZ1,cplcVWmVWmVZVZ2, & 
& cplcVWmVWmVZVZ3
...
Subroutine OneLoopVZp(g1,g2,gp,QHd,QHu,Qs,vd,vu,vS,TW,TWp,Mhh,Mhh2,MAh,               & 
& MAh2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,MFv,MFv2,MVZ,MVZ2,               & 
& MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,MSv,MSv2,cplAhhhVZp,       & 
& cplcChaChaVZpL,cplcChaChaVZpR,cplChiChiVZpL,cplChiChiVZpR,cplcFdFdVZpL,cplcFdFdVZpR,   & 
& cplcFeFeVZpL,cplcFeFeVZpR,cplcFuFuVZpL,cplcFuFuVZpR,cplcFvFvVZpL,cplcFvFvVZpR,         & 
& cplcgWmgWmVZp,cplcgWpCgWpCVZp,cplhhVZVZp,cplhhVZpVZp,cplHpmcHpmVZp,cplHpmcVWmVZp,      & 
& cplSdcSdVZp,cplSecSeVZp,cplSucSuVZp,cplSvcSvVZp,cplcVWmVWmVZp,cplAhAhVZpVZp,           & 
& cplhhhhVZpVZp,cplHpmcHpmVZpVZp,cplSdcSdVZpVZp,cplSecSeVZpVZp,cplSucSuVZpVZp,           & 
& cplSvcSvVZpVZp,cplcVWmVWmVZpVZp1,cplcVWmVWmVZpVZp2,cplcVWmVWmVZpVZp3,delta,            & 
& mass,mass2,kont)
...
Complex(dp), Intent(in) :: cplAhhhVZp(3,3),cplcChaChaVZpL(2,2),cplcChaChaVZpR(2,2),cplChiChiVZpL(6,6),           & 
& cplChiChiVZpR(6,6),cplcFdFdVZpL(3,3),cplcFdFdVZpR(3,3),cplcFeFeVZpL(3,3),              & 
& cplcFeFeVZpR(3,3),cplcFuFuVZpL(3,3),cplcFuFuVZpR(3,3),cplcFvFvVZpL(3,3),               & 
& cplcFvFvVZpR(3,3),cplcgWmgWmVZp,cplcgWpCgWpCVZp,cplhhVZVZp(3),cplhhVZpVZp(3),          & 
& cplHpmcHpmVZp(2,2),cplHpmcVWmVZp(2),cplSdcSdVZp(6,6),cplSecSeVZp(6,6),cplSucSuVZp(6,6),& 
& cplSvcSvVZp(3,3),cplcVWmVWmVZp,cplAhAhVZpVZp(3,3),cplhhhhVZpVZp(3,3),cplHpmcHpmVZpVZp(2,2),& 
& cplSdcSdVZpVZp(6,6),cplSecSeVZpVZp(6,6),cplSucSuVZpVZp(6,6),cplSvcSvVZpVZp(3,3),       & 
...
Real(dp), Intent(in) :: MHpm(2),MHpm2(2),MAh(3),MAh2(3),MChi(6),MChi2(6),MCha(2),MCha2(2),MFu(3),             & 
& MFu2(3),MFd(3),MFd2(3),MFv(3),MFv2(3),MFe(3),MFe2(3),Mhh(3),Mhh2(3),MVWm,              & 
& MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSu(6),MSu2(6),MSd(6),MSd2(6),MSv(3),MSv2(3),MSe(6),MSe2(6)

Real(dp), Intent(in) :: g2,vd,vu

Complex(dp), Intent(in) :: cplAhHpmcVWm(3,2),cplChiChacVWmL(6,2),cplChiChacVWmR(6,2),cplcFuFdcVWmL(3,3),         & 
...
& cplhhHpmcVWm(3,2),cplhhcVWmVWm(3),cplHpmcVWmVP(2),cplHpmcVWmVZ(2),cplHpmcVWmVZp(2),    & 
& cplSdcSucVWm(6,6),cplSecSvcVWm(6,3),cplcVWmVPVWm,cplcVWmVWmVZ,cplcVWmVWmVZp,           & 
...
Implicit None 
Real(dp), Intent(in) :: MAh(3),MAh2(3),MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),MFd2(3),MFe(3),               & 
& MFe2(3),MFu(3),MFu2(3),MFv(3),MFv2(3),Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MSd(6),          & 
& MSd2(6),MSe(6),MSe2(6),MSu(6),MSu2(6),MSv(3),MSv2(3),MVWm,MVWm2,MVZ,MVZ2,              & 
& MVZp,MVZp2

Complex(dp), Intent(in) :: cplAhAhVZVZp(3,3),cplAhhhVZ(3,3),cplAhhhVZp(3,3),cplcChaChaVZL(2,2),cplcChaChaVZpL(2,2),& 
...
& cplcgWmgWmVZp,cplcgWpCgWpCVZ,cplcgWpCgWpCVZp,cplChiChiVZL(6,6),cplChiChiVZpL(6,6),     & 
& cplChiChiVZpR(6,6),cplChiChiVZR(6,6),cplcHpmVWmVZ(2),cplcHpmVWmVZp(2),cplcVWmVWmVZ,    & 
& cplcVWmVWmVZp,cplcVWmVWmVZVZp1,cplcVWmVWmVZVZp2,cplcVWmVWmVZVZp3,cplhhhhVZVZp(3,3),    & 
& cplhhVZpVZp(3),cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmVZ(2,2),cplHpmcHpmVZp(2,2),        & 
...
& cplSucSuVZp(6,6),cplSucSuVZVZp(6,6),cplSvcSvVZ(3,3),cplSvcSvVZp(3,3),cplSvcSvVZVZp(3,3)
...
Implicit None 
Real(dp), Intent(in) :: MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),MFd2(3),MFe(3),MFe2(3),MFu(3),               & 
& MFu2(3),MHpm(2),MHpm2(2),MSd(6),MSd2(6),MSe(6),MSe2(6),MSu(6),MSu2(6),MSv(3),          & 
& MSv2(3),MVWm,MVWm2

Complex(dp), Intent(in) :: cplcChaChahhL(2,2,3),cplcChaChahhR(2,2,3),cplcChaChaVZL(2,2),cplcChaChaVZR(2,2),      & 
...
& cplcgWmgWmhh(3),cplcgWmgWmVZ,cplcgWpCgWpChh(3),cplcgWpCgWpCVZ,cplChiChihhL(6,6,3),     & 
& cplChiChihhR(6,6,3),cplChiChiVZL(6,6),cplChiChiVZR(6,6),cplcHpmVWmVZ(2),               & 
& cplcVWmVWmVZ,cplhhcHpmVWm(3,2),cplhhcVWmVWm(3),cplhhHpmcHpm(3,2,2),cplhhHpmcVWm(3,2),  & 
...
& cplHpmcHpmVZ,cplHpmcVWmVZ,cplSdcSdVZ,cplSecSeVZ,cplSucSuVZ,kont,res)

Implicit None 
Real(dp), Intent(in) :: MAh(3),MAh2(3),MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),MFd2(3),MFe(3),               & 
& MFe2(3),MFu(3),MFu2(3),Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MSd(6),MSd2(6),MSe(6),          & 
& MSe2(6),MSu(6),MSu2(6),MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2
...
& cplcgWmgWmAh(3),cplcgWmgWmVZ,cplcgWpCgWpCAh(3),cplcgWpCgWpCVZ,cplChiChiAhL(6,6,3),     & 
& cplChiChiAhR(6,6,3),cplChiChiVZL(6,6),cplChiChiVZR(6,6),cplcHpmVWmVZ(2),               & 
& cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmVZ(2,2),cplHpmcVWmVZ(2),cplSdcSdVZ(6,6),          & 
& cplSecSeVZ(6,6),cplSucSuVZ(6,6)
...
& MVZ,MVZ2,MVZp,MVZp2,cplAhAhhh,cplAhcHpmVWm,cplAhhhVZ,cplAhhhVZp,cplAhHpmcHpm,          & 
...
& cplChiChiAhL,cplChiChiAhR,cplChiChiVZL,cplChiChiVZR,cplcHpmVWmVZ,cplhhVZVZ,            & 
& cplhhVZVZp,cplHpmcHpmVZ,cplHpmcVWmVZ,cplSdcSdVZ,cplSecSeVZ,cplSucSuVZ,kont,res)
...
& cplSecSeVZp,cplSucSuVZp,cplSvcSvVZp,kont,res)
...
Complex(dp), Intent(in) :: cplcChaChahhL(2,2,3),cplcChaChahhR(2,2,3),cplcChaChaVZpL(2,2),cplcChaChaVZpR(2,2),    & 
...
& cplcgWmgWmhh(3),cplcgWmgWmVZp,cplcgWpCgWpChh(3),cplcgWpCgWpCVZp,cplChiChihhL(6,6,3),   & 
& cplChiChihhR(6,6,3),cplChiChiVZpL(6,6),cplChiChiVZpR(6,6),cplcHpmVWmVZp(2),            & 
& cplcVWmVWmVZp,cplhhcHpmVWm(3,2),cplhhcVWmVWm(3),cplhhHpmcHpm(3,2,2),cplhhHpmcVWm(3,2), & 
& cplhhSdcSd(3,6,6),cplhhSecSe(3,6,6),cplhhSucSu(3,6,6),cplhhSvcSv(3,3,3),               & 
& cplHpmcHpmVZp(2,2),cplHpmcVWmVZp(2),cplSdcSdVZp(6,6),cplSecSeVZp(6,6),cplSucSuVZp(6,6),& 
& cplSvcSvVZp(3,3)
...
& cplcFuFuVZpL,cplcFuFuVZpR,cplcgWmgWmAh,cplcgWmgWmVZp,cplcgWpCgWpCAh,cplcgWpCgWpCVZp,   & 
& cplChiChiAhL,cplChiChiAhR,cplChiChiVZpL,cplChiChiVZpR,cplcHpmVWmVZp,cplhhVZpVZp,       & 
& cplhhVZVZp,cplHpmcHpmVZp,cplHpmcVWmVZp,cplSdcSdVZp,cplSecSeVZp,cplSucSuVZp,kont,res)
...
Complex(dp), Intent(in) :: cplAhAhhh(3,3,3),cplAhcHpmVWm(3,2),cplAhhhVZ(3,3),cplAhhhVZp(3,3),cplAhHpmcHpm(3,2,2),& 
& cplAhHpmcVWm(3,2),cplAhSdcSd(3,6,6),cplAhSecSe(3,6,6),cplAhSucSu(3,6,6),               & 
& cplcChaChaAhL(2,2,3),cplcChaChaAhR(2,2,3),cplcChaChaVZpL(2,2),cplcChaChaVZpR(2,2),     & 
& cplcFdFdAhL(3,3,3),cplcFdFdAhR(3,3,3),cplcFdFdVZpL(3,3),cplcFdFdVZpR(3,3),             & 
& cplcFeFeAhL(3,3,3),cplcFeFeAhR(3,3,3),cplcFeFeVZpL(3,3),cplcFeFeVZpR(3,3),             & 
& cplcFuFuAhL(3,3,3),cplcFuFuAhR(3,3,3),cplcFuFuVZpL(3,3),cplcFuFuVZpR(3,3),             & 
& cplcgWmgWmAh(3),cplcgWmgWmVZp,cplcgWpCgWpCAh(3),cplcgWpCgWpCVZp,cplChiChiAhL(6,6,3),   & 
& cplChiChiAhR(6,6,3),cplChiChiVZpL(6,6),cplChiChiVZpR(6,6),cplcHpmVWmVZp(2),            & 
& cplhhVZpVZp(3),cplhhVZVZp(3),cplHpmcHpmVZp(2,2),cplHpmcVWmVZp(2),cplSdcSdVZp(6,6),     & 
& cplSecSeVZp(6,6),cplSucSuVZp(6,6)
...
Complex(dp), Intent(in) :: cplAhcHpmVWm(3,2),cplAhHpmcHpm(3,2,2),cplcChaChiVWmL(2,6),cplcChaChiVWmR(2,6),        & 
...
& cplChiChacHpmL(6,2,2),cplChiChacHpmR(6,2,2),cplcHpmVPVWm(2),cplcHpmVWmVZ(2),           & 
& cplcHpmVWmVZp(2),cplcVWmVPVWm,cplcVWmVWmVZ,cplcVWmVWmVZp,cplhhcHpmVWm(3,2),            & 
& cplhhcVWmVWm(3),cplhhHpmcHpm(3,2,2),cplHpmcHpmVP(2,2),cplHpmcHpmVZ(2,2),               & 
& cplHpmcHpmVZp(2,2),cplSdcHpmcSu(6,2,6),cplSecHpmcSv(6,2,3),cplSucSdVWm(6,6),           & 
& cplSvcSeVWm(3,6)
