...
#        self.Kgluglu_Rate = self.DRwithBlocks_HiggstoGluGlu/self.DR_HiggstoGluGlu_SMvalue
    ...
    def U1ChargeGenerator(self, *args, **kwargs):
        self.Ql1_IN = float("%0.2f" % (kwargs.get('Ql1_IN', None)))
        self.Ql2_IN = float("%0.2f" % (kwargs.get('Ql2_IN', None)))
        self.Qe1_IN = float("%0.2f" % (kwargs.get('Qe1_IN', None)))
        self.Qe2_IN = float("%0.2f" % (kwargs.get('Qe2_IN', None)))

        # Basis: Qu1, Qu2, Qu3, Qd1, Qd2, Qd3, Qe1, Qe2, Qe3, Qq1, Qq2, Qq3, Ql1, Ql2, Ql3,
        # Qv1, Qv2, Qv3, QHu, QHd, Qs, QDx, QDxbar
        self.ChargeSets = pd.read_csv('/Users/oozdal/projects/MagMoment/Scan/NonUMSSMChargeSetsv4.csv',
        ...
        if self.Ql1_IN is not None:

            if self.Ql1_IN == -0.67 and self.Ql2_IN == -0.33 and self.Qe1_IN ==  0.33  and self.Qe2_IN ==  0.00: randcho = 131
            if self.Ql1_IN == -0.71 and self.Ql2_IN == -0.14 and self.Qe1_IN ==  0.43  and self.Qe2_IN == -0.14: randcho = 234
            if self.Ql1_IN ==  0.44 and self.Ql2_IN == -0.11 and self.Qe1_IN == -1.00  and self.Qe2_IN == -0.44: randcho = 236
            if self.Ql1_IN ==  0.50 and self.Ql2_IN == -0.20 and self.Qe1_IN == -1.00  and self.Qe2_IN == -0.30: randcho = 363
            if self.Ql1_IN == -0.80 and self.Ql2_IN == -0.20 and self.Qe1_IN ==  1.00  and self.Qe2_IN ==  0.40: randcho = 400
            if self.Ql1_IN == -0.50 and self.Ql2_IN ==  0.20 and self.Qe1_IN ==  1.00  and self.Qe2_IN ==  0.30: randcho = 442
            if self.Ql1_IN == -0.44 and self.Ql2_IN ==  0.11 and self.Qe1_IN ==  1.00  and self.Qe2_IN ==  0.44: randcho = 549
            if self.Ql1_IN ==  0.71 and self.Ql2_IN ==  0.14 and self.Qe1_IN == -0.43  and self.Qe2_IN ==  0.14: randcho = 638
            if self.Ql1_IN ==  0.67 and self.Ql2_IN ==  0.33 and self.Qe1_IN == -0.33  and self.Qe2_IN ==  0.00: randcho = 771
            if self.Ql1_IN ==  0.80 and self.Ql2_IN ==  0.20 and self.Qe1_IN == -1.00  and self.Qe2_IN == -0.40: randcho = 1130
    ...
    def RunSPheno(self, LHAInput):
        ...
        p = subprocess.call(
            "./../../../packages/SPheno-4.0.4/bin/SPhenoNonUMSSMv4 "+str(LHAInput), shell=True)
    ...
    def MassBounds(self):
        ...
        if (self.MZp > 5000) and (self.MZp < 10000)	:
...
#        self.NMIX19 = self.allcontent.blocks["NMNMIX"][1, 9]
    ...
    def Calculate_DAELDAMU(self):        
        ...
        self.LoadExpData("G2_1sig", "/Users/oozdal/packages/ExpDATA/expdata/G2_1sig.csv")
        self.LoadExpData("G2_2sig", "/Users/oozdal/packages/ExpDATA/expdata/G2_2sig.csv")
        self.LoadExpData("G2_3sig", "/Users/oozdal/packages/ExpDATA/expdata/G2_3sig.csv")
    ...
    def Check_DAELDAMU_Sig(self):
        if (self.Calculate_DAELDAMU() == "1sigma" or self.Calculate_DAELDAMU() == "2sigma" or self.Calculate_DAELDAMU() == "3sigma"): return True
        else: return False
