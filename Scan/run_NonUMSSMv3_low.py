...
LHAFileFullPath = os.path.abspath("LesHouches.in.NonUMSSMv3_low")
LHAFiledirPath = os.path.dirname(LHAFileFullPath)
LHAFileName = os.path.basename(LHAFileFullPath)
######## Updated LesHouches Files Paths #######
UpdatedLHAFileFullPath = os.path.abspath("LesHouches.in.NonUMSSMv3_lowUpdated")
...
SLHAFileFullPath = os.path.abspath("SPheno.spc.NonUMSSMv3")
SLHAFiledirPath = os.path.dirname(SLHAFileFullPath)
SLHAFileName = os.path.basename(SLHAFileFullPath)
destPathforNewSLHA = os.path.dirname("SPhenoOutputs/SPheno.spc.NonUMSSMv3*")
...
fileno = len(fnmatch.filter(os.listdir(
    destPathforNewSLHA), 'SPheno.spc.NonUMSSMv3*'))
...
###########################################################################
            ...
            Tdp11IN = uniform(-5000., 5000.) # np.random.normal(5.29566845E-01, 0.1)
            Tdp22IN = uniform(-5000., 5000.) # np.random.normal(5.29566845E-01, 0.1)
            Tdp33IN = uniform(-5000., 5000.) # np.random.normal(5.29566845E-01, 0.1) 

            TepIN =  uniform(-5000, 5000) # np.random.normal(-1.66981363E+03, 10)
            TmupIN = uniform(-5000, 5000) # np.random.normal(-1.66981363E+03, 10)
            ...
            me012 = uniform(1e4, 5000**2)  # np.random.normal(7.46054725E+06, 100000)
            me022 = uniform(1e4, 5000**2)  # np.random.normal(8.41835696E+06, 100000) 
            me032 = uniform(1e4, 5000**2)  # np.random.normal(8.50282542E+06, 100000)
            ml012 = uniform(1e4, 5000**2)  # np.random.normal(2.24287690E+06, 100000) 
            ml022 = uniform(1e4, 5000**2)  # np.random.normal(8.81432832E+06, 100000)
            ml032 = uniform(1e4, 5000**2)  # np.random.normal(8.82332008E+06, 100000)

            MSD211IN = uniform(1e4, 5000**2)
            MSD222IN = uniform(1e4, 5000**2)
            MSD233IN = uniform(1e4, 5000**2)

            MSQ211IN = uniform(1e4, 5000**2)
            MSQ222IN = uniform(1e4, 5000**2)
            MSQ233IN = uniform(1e4, 5000**2)

            MSU211IN = uniform(1e4, 5000**2)
            MSU222IN = uniform(1e4, 5000**2)
            MSU233IN = uniform(1e4, 5000**2)
...
############### Assign U1 prime charges #####################
            ...
            LHA.LHAfile = LHA.LHAfile.replace(
                'MSQ211IN', str("{:.8E}".format(MSQ211IN)))
            LHA.LHAfile = LHA.LHAfile.replace(
                'MSQ222IN', str("{:.8E}".format(MSQ222IN)))
            LHA.LHAfile = LHA.LHAfile.replace(
                'MSQ233IN', str("{:.8E}".format(MSQ233IN)))

            LHA.LHAfile = LHA.LHAfile.replace(
                'MSU211IN', str("{:.8E}".format(MSU211IN)))
            LHA.LHAfile = LHA.LHAfile.replace(
                'MSU222IN', str("{:.8E}".format(MSU222IN)))
            LHA.LHAfile = LHA.LHAfile.replace(
                'MSU233IN', str("{:.8E}".format(MSU233IN)))
...
############# Check Constraints  #######################################################################
            if LHA.MassBounds() == True and LHA.CheckBPhysics() == True and LHA.G2Bound() == True: # and LHA.CheckWhoIsLSP() == True:
                fileno = len(fnmatch.filter(os.listdir(
                    destPathforNewSLHA), "SPheno.spc.NonUMSSMv3.*"))
                newfileno = fileno + 1
                LHA.NewSLHAFileName = "SPheno.spc.NonUMSSMv3."+str(newfileno)
                LHA.NewSLHAFileFullPath = "SPhenoOutputs/" + LHA.NewSLHAFileName
                LHA.RenameAndCopy(os.path.abspath(
                    "SPheno.spc.NonUMSSMv3"), LHA.NewSLHAFileFullPath)
                LHA.Erase(SLHAFileName)
            else:
                LHA.Erase(SLHAFileName)
