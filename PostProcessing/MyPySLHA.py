...
#        self.CheckConstraints = self.MZp_Bound and self.fshm2_Bound and self.Mh3_Bound and self.MDD_Bound
    ...
    def Erase(self,FilePathToErase):
        ...
        if os.path.isfile(self.FilePathToErase) == True: os.remove(self.FilePathToErase)
...
#        self.Kgluglu_Rate = self.DRwithBlocks_HiggstoGluGlu/self.DR_HiggstoGluGlu_SMvalue
    ...
    def U1ChargeGenerator(self,alpha=None):
            ...
            self.QQ_charge   =  self.alpha/9.       # 2.0412415E-01
            self.Ql1_charge  =  -3*self.alpha       # 8.75E-01
            self.Ql2_charge  =  self.alpha          # 2.25E-01
            self.Ql3_charge  =  -1*self.alpha       # -0.5E-01
            self.QHu_charge  =  self.alpha          # -4.0824829E-01
            self.QHd_charge  =  self.alpha          # -4.0824829E-01
            self.Qd_charge   =  (8*self.alpha)/9.   # 2.0412415E-01
            self.Qu_charge   =  (-10*self.alpha)/9. # 2.0412415E-01
            self.Qe1_charge  =  4*self.alpha        # -0.75
            self.Qe2_charge  =  0.                  # -0.125
            self.Qe3_charge  =  0.                  # 0.125
            self.Qs_charge   = -2*self.alpha        # 8.1649658E-01
...
#        p = subprocess.call("./../../../../../home/oo4g19/softwares/SPheno-4.0.4/bin/SPhenosec_UMSSM "+str(LHAInput),shell=True)
        p = subprocess.call("./../../../packages/SPheno-4.0.4/bin/SPhenoNonUMSSM "+str(LHAInput),shell=True)
	...
	self.ZZmixing = abs(self.allcontent.blocks["ANGLES"][10])
        ...
        if (self.mh1 >= 122 and self.mh1 <= 128) or (self.mh2 >= 122 and self.mh2 <= 128) or (self.mh3 >= 122 and self.mh3 <= 128): self.SMlikeHiggsMass = True
	if (self.Gluino >= 1800)                        : self.GluinoBound   = True

	if (self.Cha1 >= 103.5)                         : self.Cha1Bound     = True

        if (self.MZp > 1000) and (self.MZp < 6000)	: self.ZpMassBound   = True
	if (self.ZZmixing <= 1e-3)                      : self.ZZmixingBound = True

        self.CheckConstraints = self.SMlikeHiggsMass and self.GluinoBound and self.Cha1Bound and self.ZpMassBound # and self.ZZmixingBound
        return self.CheckConstraints
