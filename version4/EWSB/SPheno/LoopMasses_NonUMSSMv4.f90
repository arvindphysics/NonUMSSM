...
! ------------------------------------------------------------------------------  
! File created at 15:59 on 21.3.2021   
! ----------------------------------------------------------------------  
 
 
Module LoopMasses_NonUMSSMv4 
...
Use StandardModel 
Use Tadpoles_NonUMSSMv4 
 Use EffectivePotential_NonUMSSMv4 
 Use Pole2L_NonUMSSMv4 
...
& cplAhAhUSvcUSv(3,3,6,6),cplAhAhVZpVZp(3,3),cplAhAhVZVZ(3,3),cplAhAhVZVZp(3,3),         & 
& cplAhcUHpmVWm(3,2),cplAhhhVZ(3,3),cplAhhhVZp(3,3),cplAhHpmcUHpm(3,2,2),cplAhHpmcVWm(3,2),& 
& cplAhSdcUSd(3,6,6),cplAhSecUSe(3,6,6),cplAhSucUSu(3,6,6),cplAhSvcUSv(3,6,6),           & 
& cplAhUhhhh(3,3,3),cplAhUhhVZ(3,3),cplAhUhhVZp(3,3),cplcChaChaUAhL(2,2,3),              & 
...
& cplcChaFecUSvR(2,3,6),cplcChaUChiHpmL(2,6,2),cplcChaUChiHpmR(2,6,2),cplcChaUChiVWmL(2,6),& 
& cplcChaUChiVWmR(2,6),cplcFdFdUAhL(3,3,3),cplcFdFdUAhR(3,3,3),cplcFdFdUhhL(3,3,3),      & 
...
& cplcFdGluSdR(3,6),cplcFdUChiSdL(3,6,6),cplcFdUChiSdR(3,6,6),cplcFeFeUAhL(3,3,3),       & 
...
& cplcFeUChiSeL(3,6,6),cplcFeUChiSeR(3,6,6),cplcFuFdcUHpmL(3,3,2),cplcFuFdcUHpmR(3,3,2), & 
...
& cplcFuFuVZR(3,3),cplcFuGluSuL(3,6),cplcFuGluSuR(3,6),cplcFuUChiSuL(3,6,6),             & 
& cplcFuUChiSuR(3,6,6),cplcFvFecUHpmL(3,3,2),cplcFvFecUHpmR(3,3,2),cplcFvFecVWmL(3,3),   & 
& cplcFvFecVWmR(3,3),cplcFvFvUAhL(3,3,3),cplcFvFvUAhR(3,3,3),cplcFvFvUhhL(3,3,3),        & 
& cplcFvFvUhhR(3,3,3),cplcFvFvVZL(3,3),cplcFvFvVZpL(3,3),cplcFvFvVZpR(3,3),              & 
& cplcFvFvVZR(3,3),cplcFvUChiSvL(3,6,6),cplcFvUChiSvR(3,6,6),cplcgAgWmcVWm,              & 
& cplcgGgGVG,cplcgWmgWmUAh(3),cplcgWmgWmUhh(3),cplcgWmgWmVP,cplcgWmgWmVZ,cplcgWmgWmVZp,  & 
& cplcgWmgZpUHpm(2),cplcgWmgZUHpm(2),cplcgWpCgAcVWm,cplcgWpCgWpCUAh(3),cplcgWpCgWpCUhh(3),& 
...
& cplcgZgZpUhh(3),cplcgZgZUhh(3),cplcgZpgWmcUHpm(2),cplcgZpgWmcVWm,cplcgZpgWpCUHpm(2),   & 
& cplcgZpgZpUhh(3),cplcgZpgZUhh(3),cplChaFucUSdL(2,3,6),cplChaFucUSdR(2,3,6),            & 
& cplChaFvcUSeL(2,3,6),cplChaFvcUSeR(2,3,6),cplChiChacUHpmL(6,2,2),cplChiChacUHpmR(6,2,2),& 
& cplChiChacVWmL(6,2),cplChiChacVWmR(6,2),cplChiChiUAhL(6,6,3),cplChiChiUAhR(6,6,3),     & 
& cplChiChiUhhL(6,6,3),cplChiChiUhhR(6,6,3),cplChiChiVZL(6,6),cplChiChiVZpL(6,6),        & 
& cplChiChiVZpR(6,6),cplChiChiVZR(6,6),cplChiFdcUSdL(6,3,6),cplChiFdcUSdR(6,3,6),        & 
& cplChiFecUSeL(6,3,6),cplChiFecUSeR(6,3,6),cplChiFucUSuL(6,3,6),cplChiFucUSuR(6,3,6)

Complex(dp) :: cplChiFvcUSvL(6,3,6),cplChiFvcUSvR(6,3,6),cplcHpmVPVWm(2),cplcHpmVWmVZ(2),             & 
& cplcHpmVWmVZp(2),cplcUChacFuSdL(2,3,6),cplcUChacFuSdR(2,3,6),cplcUChacFvSeL(2,3,6),    & 
& cplcUChacFvSeR(2,3,6),cplcUChaChaAhL(2,2,3),cplcUChaChaAhR(2,2,3),cplcUChaChahhL(2,2,3),& 
& cplcUChaChahhR(2,2,3),cplcUChaChaVPL(2,2),cplcUChaChaVPR(2,2),cplcUChaChaVZL(2,2),     & 
& cplcUChaChaVZpL(2,2),cplcUChaChaVZpR(2,2),cplcUChaChaVZR(2,2),cplcUChaChiHpmL(2,6,2),  & 
& cplcUChaChiHpmR(2,6,2),cplcUChaChiVWmL(2,6),cplcUChaChiVWmR(2,6),cplcUChaFdcSuL(2,3,6),& 
& cplcUChaFdcSuR(2,3,6),cplcUChaFecSvL(2,3,6),cplcUChaFecSvR(2,3,6),cplcUFdChaSuL(3,2,6),& 
& cplcUFdChaSuR(3,2,6),cplcUFdChiSdL(3,6,6),cplcUFdChiSdR(3,6,6),cplcUFdFdAhL(3,3,3),    & 
...
& cplcUFdGluSdR(3,6),cplcUFeChaSvL(3,2,6),cplcUFeChaSvR(3,2,6),cplcUFeChiSeL(3,6,6),     & 
& cplcUFeChiSeR(3,6,6),cplcUFeFeAhL(3,3,3),cplcUFeFeAhR(3,3,3),cplcUFeFehhL(3,3,3),      & 
...
& cplcUFeFvHpmR(3,3,2),cplcUFeFvVWmL(3,3),cplcUFeFvVWmR(3,3),cplcUFuChiSuL(3,6,6),       & 
& cplcUFuChiSuR(3,6,6),cplcUFuFdcHpmL(3,3,2),cplcUFuFdcHpmR(3,3,2),cplcUFuFdcVWmL(3,3),  & 
...
& cplcUFuFuVZR(3,3),cplcUFuGluSuL(3,6),cplcUFuGluSuR(3,6),cplcUFvChiSvL(3,6,6),          & 
& cplcUFvChiSvR(3,6,6),cplcUFvFecHpmL(3,3,2),cplcUFvFecHpmR(3,3,2),cplcUFvFecVWmL(3,3),  & 
& cplcUFvFecVWmR(3,3),cplcUFvFvAhL(3,3,3),cplcUFvFvAhR(3,3,3),cplcUFvFvhhL(3,3,3),       & 
& cplcUFvFvhhR(3,3,3),cplcUFvFvVZL(3,3),cplcUFvFvVZpL(3,3),cplcUFvFvVZpR(3,3),           & 
...
& cplGluFucUSuL(3,6),cplGluFucUSuR(3,6),cplGluGluVGL,cplGluGluVGR,cplhhcUHpmVWm(3,2),    & 
& cplhhcVWmVWm(3),cplhhhhcVWmVWm(3,3),cplhhhhUHpmcUHpm(3,3,2,2),cplhhhhUSdcUSd(3,3,6,6), & 
& cplhhhhUSecUSe(3,3,6,6),cplhhhhUSucUSu(3,3,6,6),cplhhhhUSvcUSv(3,3,6,6),               & 
& cplhhhhVZpVZp(3,3),cplhhhhVZVZ(3,3),cplhhhhVZVZp(3,3),cplhhHpmcUHpm(3,2,2),            & 
& cplhhHpmcVWm(3,2),cplhhSdcUSd(3,6,6),cplhhSecUSe(3,6,6),cplhhSucUSu(3,6,6)

Complex(dp) :: cplhhSvcUSv(3,6,6),cplhhVZpVZp(3),cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmcVWmVWm(2,2),   & 
...
& cplSvcUSvVZp(6,6),cplUAhAhAh(3,3,3),cplUAhAhhh(3,3,3),cplUAhhhhh(3,3,3),               & 
& cplUAhhhVZ(3,3),cplUAhhhVZp(3,3),cplUAhHpmcHpm(3,2,2),cplUAhHpmcVWm(3,2),              & 
& cplUAhSdcSd(3,6,6),cplUAhSecSe(3,6,6),cplUAhSucSu(3,6,6),cplUAhSvcSv(3,6,6),           & 
...
& cplUAhUAhSvcSv(3,3,6,6),cplUAhUAhVZpVZp(3,3),cplUAhUAhVZVZ(3,3),cplUChiChacHpmL(6,2,2),& 
& cplUChiChacHpmR(6,2,2),cplUChiChacVWmL(6,2),cplUChiChacVWmR(6,2),cplUChiChiAhL(6,6,3), & 
& cplUChiChiAhR(6,6,3),cplUChiChihhL(6,6,3),cplUChiChihhR(6,6,3),cplUChiChiVZL(6,6),     & 
& cplUChiChiVZpL(6,6),cplUChiChiVZpR(6,6),cplUChiChiVZR(6,6),cplUChiFdcSdL(6,3,6),       & 
& cplUChiFdcSdR(6,3,6),cplUChiFecSeL(6,3,6),cplUChiFecSeR(6,3,6),cplUChiFucSuL(6,3,6),   & 
& cplUChiFucSuR(6,3,6),cplUChiFvcSvL(6,3,6),cplUChiFvcSvR(6,3,6),cplUhhcVWmVWm(3),       & 
& cplUhhhhhh(3,3,3),cplUhhHpmcHpm(3,2,2),cplUhhHpmcVWm(3,2),cplUhhSdcSd(3,6,6),          & 
& cplUhhSecSe(3,6,6),cplUhhSucSu(3,6,6),cplUhhSvcSv(3,6,6),cplUhhUhhcVWmVWm(3,3),        & 
& cplUhhUhhhhhh(3,3,3,3),cplUhhUhhHpmcHpm(3,3,2,2),cplUhhUhhSdcSd(3,3,6,6),              & 
& cplUhhUhhSecSe(3,3,6,6),cplUhhUhhSucSu(3,3,6,6),cplUhhUhhSvcSv(3,3,6,6)

Complex(dp) :: cplUhhUhhVZpVZp(3,3),cplUhhUhhVZVZ(3,3),cplUhhVZpVZp(3),cplUhhVZVZ(3),cplUhhVZVZp(3),  & 
...
& mvR032,ms2,M1,M2,M3,M4,Tep,Tmup,GenerationMixing,kont)
...
& gp,ZH,ZA,TWp,ZN,ZVL,ZVR,vS,ZV,ZDL,ZUL,cplcChaChaVPL,cplcChaChaVPR,cplcFdFdVPL,         & 
...
vu=(TanBetaQ*vSM)/Sqrt(1 + TanBetaQ**2) 
End if 
Call SolveTadpoleEquations(g1,g2,g3,gp,Yd,Ye11,Ye22,Ye33,lam,Yv11,Yv22,               & 
...
& Tep,Tmup,vd,vu,vS,(/ ZeroC, ZeroC, ZeroC /))
...
& gp,ZN,ZDL,ZDR,g3,pG,vd,ZH,Tu,ZU,ZP,TW,TWp,Ye11,Ye22,Ye33,ZE,ZV,Yv11,TYv11,             & 
& Yv22,TYv22,Yv33,TYv33,ZVL,ZVR,ZEL,ZER,TYe11,TYe22,TYe33,Tep,Tmup,Tlam,cplAhSdcUSd,     & 
& cplChaFucUSdL,cplChaFucUSdR,cplChiFdcUSdL,cplChiFdcUSdR,cplGluFdcUSdL,cplGluFdcUSdR,   & 
& cplhhSdcUSd,cplHpmSucUSd,cplSdcUSdVG,cplSdcUSdVP,cplSdcUSdVZ,cplSdcUSdVZp,             & 
& cplSucUSdVWm,cplAhAhUSdcUSd,cplhhhhUSdcUSd,cplHpmUSdcHpmcUSd,cplUSdSdcUSdcSd,          & 
& cplUSdSecUSdcSe,cplUSdSucUSdcSu,cplUSdSvcUSdcSv,cplUSdcUSdVGVG,cplUSdcUSdVPVP,         & 
& cplUSdcUSdcVWmVWm,cplUSdcUSdVZVZ,cplUSdcUSdVZpVZp,cplAhSvcUSv,cplChiFvcUSvL,           & 
...
& cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,cplcUFvFvAhL,cplcUFvFvAhR,cplcUFvChiSvL,               & 
...
& cplcChacUFvSeL,cplcChacUFvSeR,cplAhSucUSu,cplChiFucUSuL,cplChiFucUSuR,cplcChaFdcUSuL,  & 
...
& cplAhSecUSe,cplChaFvcUSeL,cplChaFvcUSeR,cplChiFecUSeL,cplChiFecUSeR,cplhhSecUSe,       & 
& cplHpmSvcUSe,cplSecUSeVP,cplSecUSeVZ,cplSecUSeVZp,cplSvcUSeVWm,cplAhAhUSecUSe,         & 
& cplhhhhUSecUSe,cplHpmUSecHpmcUSe,cplSdUSecSdcUSe,cplUSeSecUSecSe,cplUSeSucUSecSu,      & 
& cplUSeSvcUSecSv,cplUSecUSeVPVP,cplUSecUSecVWmVWm,cplUSecUSeVZVZ,cplUSecUSeVZpVZp,      & 
& cplAhAhUhh,cplAhUhhhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,            & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcFvFvUhhL,cplcFvFvUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,     & 
...
& cplUhhUhhVZVZ,cplUhhUhhVZpVZp,cplUAhAhAh,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,     & 
...
& MSv,MSv2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,cplAhAhUhh,cplcChaChaUhhL,cplcChaChaUhhR,      & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcFvFvUhhL,cplcFvFvUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,     & 
& cplcgZgZUhh,cplcgZpgZpUhh,cplUhhhhhh,cplUhhHpmcHpm,cplUhhSdcSd,cplUhhSecSe,            & 
& cplUhhSucSu,cplUhhSvcSv,cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZpVZp,Tad1Loop(1:3))
...
& Ye11,Ye22,Ye33,lam,Yv11,Yv22,Yv33,Yu,Td,TYe11,TYe22,TYe33,Tlam,TYv11,TYv22,            & 
& TYv33,Tu,mq2,ml012,ml022,ml032,mHd2,mHu2,md2,mu2,me012,me022,me032,mvR012,             & 
& mvR022,mvR032,ms2,M1,M2,M3,M4,Tep,Tmup,kont)
...
& Yv11,Yv22,Yv33,Yu,Td,TYe11,TYe22,TYe33,Tlam,TYv11,TYv22,TYv33,Tu,mq2,ml012,            & 
& ml022,ml032,mHd2,mHu2,md2,mu2,me012,me022,me032,mvR012,mvR022,mvR032,ms2,              & 
& M1,M2,M3,M4,Tep,Tmup,kont,ti_ep2L,Pi2S_EffPot,PiP2S_EffPot)

Else 
Call CalculatePi2S(0._dp,vd,vu,vS,g1,g2,g3,gp,Yd,Ye11,Ye22,Ye33,lam,Yv11,             & 
...
& M3,M4,Tep,Tmup,kont,ti_ep2L,Pi2S_EffPot,PiP2S_EffPot)
...
mHu22L = mHu2
ms22L = ms2
Call OneLoopSd(g1,g2,gp,Qq,QHd,QHu,Qd,Qs,Yd,Td,lam,mq2,md2,vd,vu,vS,MSd,              & 
& MSd2,MAh,MAh2,MFu,MFu2,MCha,MCha2,MFd,MFd2,MChi,MChi2,MGlu,MGlu2,Mhh,Mhh2,             & 
& MSu,MSu2,MHpm,MHpm2,MVZ,MVZ2,MVZp,MVZp2,MVWm,MVWm2,MSe,MSe2,MSv,MSv2,cplAhSdcUSd,      & 
& cplChaFucUSdL,cplChaFucUSdR,cplChiFdcUSdL,cplChiFdcUSdR,cplGluFdcUSdL,cplGluFdcUSdR,   & 
& cplhhSdcUSd,cplHpmSucUSd,cplSdcUSdVG,cplSdcUSdVP,cplSdcUSdVZ,cplSdcUSdVZp,             & 
& cplSucUSdVWm,cplAhAhUSdcUSd,cplhhhhUSdcUSd,cplHpmUSdcHpmcUSd,cplUSdSdcUSdcSd,          & 
& cplUSdSecUSdcSe,cplUSdSucUSdcSu,cplUSdSvcUSdcSv,cplUSdcUSdVGVG,cplUSdcUSdVPVP,         & 
& cplUSdcUSdcVWmVWm,cplUSdcUSdVZVZ,cplUSdcUSdVZpVZp,0.1_dp*delta_mass,MSd_1L,            & 
& MSd2_1L,ZD_1L,kont)

Call OneLoopSv(g1,g2,gp,Ql1,Ql2,Ql3,QHd,QHu,Qv1,Qv2,Qv3,Qs,lam,Yv11,TYv11,            & 
& Yv22,TYv22,Yv33,TYv33,ml012,ml022,ml032,mvR012,mvR022,mvR032,vd,vu,vS,MSv,             & 
& MSv2,MAh,MAh2,MFv,MFv2,MChi,MChi2,MCha,MCha2,MFe,MFe2,Mhh,Mhh2,MHpm,MHpm2,             & 
& MSe,MSe2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSd,MSd2,MSu,MSu2,cplAhSvcUSv,cplChiFvcUSvL,   & 
& cplChiFvcUSvR,cplcChaFecUSvL,cplcChaFecUSvR,cplhhSvcUSv,cplSecHpmcUSv,cplSecUSvcVWm,   & 
& cplSvcUSvVZ,cplSvcUSvVZp,cplAhAhUSvcUSv,cplhhhhUSvcUSv,cplHpmUSvcHpmcUSv,              & 
& cplSdUSvcSdcUSv,cplSeUSvcSecUSv,cplSuUSvcSucUSv,cplUSvSvcUSvcSv,cplUSvcUSvcVWmVWm,     & 
& cplUSvcUSvVZVZ,cplUSvcUSvVZpVZp,0.1_dp*delta_mass,MSv_1L,MSv2_1L,ZV_1L,kont)

Call OneLoopFv(Yv11,Yv22,Yv33,vu,MFv,MFv2,MAh,MAh2,MSv,MSv2,MChi,MChi2,               & 
& MHpm,MHpm2,MFe,MFe2,MVWm,MVWm2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MCha,MCha2,MSe,            & 
& MSe2,cplcUFvFvAhL,cplcUFvFvAhR,cplcUFvChiSvL,cplcUFvChiSvR,cplcUFvFecHpmL,             & 
& cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvhhL,cplcUFvFvhhR,cplcUFvFvVZL,   & 
& cplcUFvFvVZR,cplcUFvFvVZpL,cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,0.1_dp*delta_mass,& 
& MFv_1L,MFv2_1L,ZVL_1L,ZVR_1L,kont)
...
& MAh,MAh2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,              & 
& MFe2,MFu,MFu2,MFv,MFv2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,MSu2,               & 
& MSv,MSv2,cplAhAhUhh,cplAhUhhhh,cplAhUhhVZ,cplAhUhhVZp,cplcChaChaUhhL,cplcChaChaUhhR,   & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcFvFvUhhL,cplcFvFvUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,     & 
& cplcgZgZUhh,cplcgZpgZUhh,cplcgZgZpUhh,cplcgZpgZpUhh,cplUhhhhhh,cplUhhHpmcHpm,          & 
& cplUhhHpmcVWm,cplUhhSdcSd,cplUhhSecSe,cplUhhSucSu,cplUhhSvcSv,cplUhhcVWmVWm,           & 
& cplUhhVZVZ,cplUhhVZVZp,cplUhhVZpVZp,cplAhAhUhhUhh,cplUhhUhhhhhh,cplUhhUhhHpmcHpm,      & 
& cplUhhUhhSdcSd,cplUhhUhhSecSe,cplUhhUhhSucSu,cplUhhUhhSvcSv,cplUhhUhhcVWmVWm,          & 
& cplUhhUhhVZVZ,cplUhhUhhVZpVZp,0.1_dp*delta_mass,Mhh_1L,Mhh2_1L,ZH_1L,kont)

If (TwoLoopMethod.gt.2) Then 
Call OneLoopAh(g1,g2,gp,QHd,QHu,Qs,lam,Tlam,mHd22L,mHu22L,ms22L,vd,vu,vS,             & 
& TW,TWp,MAh,MAh2,Mhh,Mhh2,MCha,MCha2,MChi,MChi2,MFd,MFd2,MFe,MFe2,MFu,MFu2,             & 
& MFv,MFv2,MVZ,MVZ2,MVZp,MVZp2,MHpm,MHpm2,MVWm,MVWm2,MSd,MSd2,MSe,MSe2,MSu,              & 
& MSu2,MSv,MSv2,cplUAhAhAh,cplUAhAhhh,cplcChaChaUAhL,cplcChaChaUAhR,cplChiChiUAhL,       & 
...
& MSu2,MSv,MSv2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,cplAhAhUhh,cplcChaChaUhhL,cplcChaChaUhhR, & 
& cplChiChiUhhL,cplChiChiUhhR,cplcFdFdUhhL,cplcFdFdUhhR,cplcFeFeUhhL,cplcFeFeUhhR,       & 
& cplcFuFuUhhL,cplcFuFuUhhR,cplcFvFvUhhL,cplcFvFvUhhR,cplcgWmgWmUhh,cplcgWpCgWpCUhh,     & 
& cplcgZgZUhh,cplcgZpgZpUhh,cplUhhhhhh,cplUhhHpmcHpm,cplUhhSdcSd,cplUhhSecSe,            & 
& cplUhhSucSu,cplUhhSvcSv,cplUhhcVWmVWm,cplUhhVZVZ,cplUhhVZpVZp,tadpoles)
...
& cplcFeFeUhhR(3,3,3),cplcFuFuUhhL(3,3,3),cplcFuFuUhhR(3,3,3),cplcFvFvUhhL(3,3,3),       & 
& cplcFvFvUhhR(3,3,3),cplcgWmgWmUhh(3),cplcgWpCgWpCUhh(3),cplcgZgZUhh(3),cplcgZpgZpUhh(3),& 
& cplUhhhhhh(3,3,3),cplUhhHpmcHpm(3,2,2),cplUhhSdcSd(3,6,6),cplUhhSecSe(3,6,6),          & 
& cplUhhSucSu(3,6,6),cplUhhSvcSv(3,6,6),cplUhhcVWmVWm(3),cplUhhVZVZ(3),cplUhhVZpVZp(3)
...
Subroutine OneLoopSd(g1,g2,gp,Qq,QHd,QHu,Qd,Qs,Yd,Td,lam,mq2,md2,vd,vu,               & 
& vS,MSd,MSd2,MAh,MAh2,MFu,MFu2,MCha,MCha2,MFd,MFd2,MChi,MChi2,MGlu,MGlu2,               & 
...
Real(dp), Intent(in) :: g1,g2,gp,Qq,QHd,QHu,Qd,Qs,vd,vu,vS
...
Subroutine OneLoopSv(g1,g2,gp,Ql1,Ql2,Ql3,QHd,QHu,Qv1,Qv2,Qv3,Qs,lam,Yv11,            & 
& TYv11,Yv22,TYv22,Yv33,TYv33,ml012,ml022,ml032,mvR012,mvR022,mvR032,vd,vu,              & 
& vS,MSv,MSv2,MAh,MAh2,MFv,MFv2,MChi,MChi2,MCha,MCha2,MFe,MFe2,Mhh,Mhh2,MHpm,            & 
& MHpm2,MSe,MSe2,MVWm,MVWm2,MVZ,MVZ2,MVZp,MVZp2,MSd,MSd2,MSu,MSu2,cplAhSvcUSv,           & 
...
Implicit None 
Real(dp), Intent(in) :: MSv(6),MSv2(6),MAh(3),MAh2(3),MFv(3),MFv2(3),MChi(6),MChi2(6),MCha(2),MCha2(2),       & 
& MFe(3),MFe2(3),Mhh(3),Mhh2(3),MHpm(2),MHpm2(2),MSe(6),MSe2(6),MVWm,MVWm2,              & 
& MVZ,MVZ2,MVZp,MVZp2,MSd(6),MSd2(6),MSu(6),MSu2(6)

Real(dp), Intent(in) :: g1,g2,gp,Ql1,Ql2,Ql3,QHd,QHu,Qv1,Qv2,Qv3,Qs,ml012,ml022,ml032,mvR012,mvR022,          & 
& mvR032,vd,vu,vS

Complex(dp), Intent(in) :: lam,Yv11,TYv11,Yv22,TYv22,Yv33,TYv33

Complex(dp), Intent(in) :: cplAhSvcUSv(3,6,6),cplChiFvcUSvL(6,3,6),cplChiFvcUSvR(6,3,6),cplcChaFecUSvL(2,3,6),   & 
& cplcChaFecUSvR(2,3,6),cplhhSvcUSv(3,6,6),cplSecHpmcUSv(6,2,6),cplSecUSvcVWm(6,6),      & 
& cplSvcUSvVZ(6,6),cplSvcUSvVZp(6,6),cplAhAhUSvcUSv(3,3,6,6),cplhhhhUSvcUSv(3,3,6,6),    & 
...
mat2a(1,1) = mat2a(1,1)+(vu**2*Yv11*Conjg(Yv11))/2._dp
mat2a(1,2) = 0._dp 
mat2a(1,3) = 0._dp 
mat2a(1,4) = 0._dp 
mat2a(1,4) = mat2a(1,4)-(vd*vS*lam*Conjg(Yv11))/2._dp
mat2a(1,4) = mat2a(1,4)+(vu*Conjg(TYv11))/sqrt(2._dp)
mat2a(1,5) = 0._dp 
mat2a(1,6) = 0._dp 
mat2a(2,2) = 0._dp 
...
mat2a(2,2) = mat2a(2,2)+(vu**2*Yv22*Conjg(Yv22))/2._dp
mat2a(2,3) = 0._dp 
mat2a(2,4) = 0._dp 
mat2a(2,5) = 0._dp 
mat2a(2,5) = mat2a(2,5)-(vd*vS*lam*Conjg(Yv22))/2._dp
mat2a(2,5) = mat2a(2,5)+(vu*Conjg(TYv22))/sqrt(2._dp)
mat2a(2,6) = 0._dp 
mat2a(3,3) = 0._dp 
...
mat2a(3,3) = mat2a(3,3)+(vu**2*Yv33*Conjg(Yv33))/2._dp
mat2a(3,4) = 0._dp 
mat2a(3,5) = 0._dp 
mat2a(3,6) = 0._dp 
mat2a(3,6) = mat2a(3,6)-(vd*vS*lam*Conjg(Yv33))/2._dp
mat2a(3,6) = mat2a(3,6)+(vu*Conjg(TYv33))/sqrt(2._dp)
mat2a(4,4) = 0._dp 
mat2a(4,4) = mat2a(4,4)+mvR012
mat2a(4,4) = mat2a(4,4)+(gp**2*QHd*Qv1*vd**2)/2._dp
mat2a(4,4) = mat2a(4,4)+(gp**2*Qs*Qv1*vS**2)/2._dp
mat2a(4,4) = mat2a(4,4)+(gp**2*QHu*Qv1*vu**2)/2._dp
mat2a(4,4) = mat2a(4,4)+(vu**2*Yv11*Conjg(Yv11))/2._dp
mat2a(4,5) = 0._dp 
mat2a(4,6) = 0._dp 
mat2a(5,5) = 0._dp 
mat2a(5,5) = mat2a(5,5)+mvR022
mat2a(5,5) = mat2a(5,5)+(gp**2*QHd*Qv2*vd**2)/2._dp
mat2a(5,5) = mat2a(5,5)+(gp**2*Qs*Qv2*vS**2)/2._dp
mat2a(5,5) = mat2a(5,5)+(gp**2*QHu*Qv2*vu**2)/2._dp
mat2a(5,5) = mat2a(5,5)+(vu**2*Yv22*Conjg(Yv22))/2._dp
mat2a(5,6) = 0._dp 
mat2a(6,6) = 0._dp 
mat2a(6,6) = mat2a(6,6)+mvR032
mat2a(6,6) = mat2a(6,6)+(gp**2*QHd*Qv3*vd**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(gp**2*Qs*Qv3*vS**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(gp**2*QHu*Qv3*vu**2)/2._dp
mat2a(6,6) = mat2a(6,6)+(vu**2*Yv33*Conjg(Yv33))/2._dp
...
Subroutine OneLoopFv(Yv11,Yv22,Yv33,vu,MFv,MFv2,MAh,MAh2,MSv,MSv2,MChi,               & 
& MChi2,MHpm,MHpm2,MFe,MFe2,MVWm,MVWm2,Mhh,Mhh2,MVZ,MVZ2,MVZp,MVZp2,MCha,MCha2,          & 
& MSe,MSe2,cplcUFvFvAhL,cplcUFvFvAhR,cplcUFvChiSvL,cplcUFvChiSvR,cplcUFvFecHpmL,         & 
& cplcUFvFecHpmR,cplcUFvFecVWmL,cplcUFvFecVWmR,cplcUFvFvhhL,cplcUFvFvhhR,cplcUFvFvVZL,   & 
& cplcUFvFvVZR,cplcUFvFvVZpL,cplcUFvFvVZpR,cplcChacUFvSeL,cplcChacUFvSeR,delta,          & 
& MFv_1L,MFv2_1L,ZVL_1L,ZVR_1L,ierr)

Implicit None 
Real(dp), Intent(in) :: MFv(3),MFv2(3),MAh(3),MAh2(3),MSv(6),MSv2(6),MChi(6),MChi2(6),MHpm(2),MHpm2(2),       & 
& MFe(3),MFe2(3),MVWm,MVWm2,Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MCha(2),MCha2(2),         & 
& MSe(6),MSe2(6)

Real(dp), Intent(in) :: vu

Complex(dp), Intent(in) :: Yv11,Yv22,Yv33

Complex(dp), Intent(in) :: cplcUFvFvAhL(3,3,3),cplcUFvFvAhR(3,3,3),cplcUFvChiSvL(3,6,6),cplcUFvChiSvR(3,6,6),    & 
& cplcUFvFecHpmL(3,3,2),cplcUFvFecHpmR(3,3,2),cplcUFvFecVWmL(3,3),cplcUFvFecVWmR(3,3),   & 
& cplcUFvFvhhL(3,3,3),cplcUFvFvhhR(3,3,3),cplcUFvFvVZL(3,3),cplcUFvFvVZR(3,3),           & 
...
mat1a(1,1) = 0._dp 
mat1a(1,1) = mat1a(1,1)+(vu*Yv11)/sqrt(2._dp)
mat1a(1,2) = 0._dp 
mat1a(1,3) = 0._dp 
mat1a(2,1) = 0._dp 
mat1a(2,2) = 0._dp 
mat1a(2,2) = mat1a(2,2)+(vu*Yv22)/sqrt(2._dp)
mat1a(2,3) = 0._dp 
mat1a(3,1) = 0._dp 
mat1a(3,2) = 0._dp 
mat1a(3,3) = 0._dp 
mat1a(3,3) = mat1a(3,3)+(vu*Yv33)/sqrt(2._dp)
...
Real(dp), Intent(in) :: g1,g2,gp,Qq,QHd,QHu,Qu,Qs,vd,vu,vS
...
& MChi(6),MChi2(6),Mhh(3),Mhh2(3),MSv(6),MSv2(6),MHpm(2),MHpm2(2),MVZ,MVZ2,              & 
& MVZp,MVZp2,MVWm,MVWm2,MSd(6),MSd2(6),MSu(6),MSu2(6)

Real(dp), Intent(in) :: g1,g2,gp,Ql1,Ql2,Ql3,QHd,QHu,Qe1,Qe2,Qe3,Qs,ml012,ml022,ml032,me012,me022,            & 
& me032,vd,vu,vS

Complex(dp), Intent(in) :: Ye11,TYe11,Ye22,TYe22,Ye33,TYe33,lam,Tep,Tmup

Complex(dp), Intent(in) :: cplAhSecUSe(3,6,6),cplChaFvcUSeL(2,3,6),cplChaFvcUSeR(2,3,6),cplChiFecUSeL(6,3,6),    & 
& cplChiFecUSeR(6,3,6),cplhhSecUSe(3,6,6),cplHpmSvcUSe(2,6,6),cplSecUSeVP(6,6),          & 
& cplSecUSeVZ(6,6),cplSecUSeVZp(6,6),cplSvcUSeVWm(6,6),cplAhAhUSecUSe(3,3,6,6),          & 
...
& cplUhhVZVZ,cplUhhVZVZp,cplUhhVZpVZp,cplAhAhUhhUhh,cplUhhUhhhhhh,cplUhhUhhHpmcHpm,      & 
& cplUhhUhhSdcSd,cplUhhUhhSecSe,cplUhhUhhSucSu,cplUhhUhhSvcSv,cplUhhUhhcVWmVWm,          & 
& cplUhhUhhVZVZ,cplUhhUhhVZpVZp,delta,mass,mass2,RS,kont)

Implicit None 
Real(dp), Intent(in) :: MAh(3),MAh2(3),Mhh(3),Mhh2(3),MVZ,MVZ2,MVZp,MVZp2,MCha(2),MCha2(2),MChi(6),           & 
& MChi2(6),MFd(3),MFd2(3),MFe(3),MFe2(3),MFu(3),MFu2(3),MFv(3),MFv2(3),MHpm(2),          & 
& MHpm2(2),MVWm,MVWm2,MSd(6),MSd2(6),MSe(6),MSe2(6),MSu(6),MSu2(6),MSv(6),MSv2(6)

Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,mHd2,mHu2,ms2,vd,vu,vS

Complex(dp), Intent(in) :: lam,Tlam

Complex(dp), Intent(in) :: cplAhAhUhh(3,3,3),cplAhUhhhh(3,3,3),cplAhUhhVZ(3,3),cplAhUhhVZp(3,3),cplcChaChaUhhL(2,2,3),& 
...
& cplcFuFuUhhR(3,3,3),cplcFvFvUhhL(3,3,3),cplcFvFvUhhR(3,3,3),cplcgWmgWmUhh(3),          & 
& cplcgWpCgWpCUhh(3),cplcgZgZUhh(3),cplcgZpgZUhh(3),cplcgZgZpUhh(3),cplcgZpgZpUhh(3),    & 
& cplUhhhhhh(3,3,3),cplUhhHpmcHpm(3,2,2),cplUhhHpmcVWm(3,2),cplUhhSdcSd(3,6,6),          & 
& cplUhhSecSe(3,6,6),cplUhhSucSu(3,6,6),cplUhhSvcSv(3,6,6),cplUhhcVWmVWm(3),             & 
& cplUhhVZVZ(3),cplUhhVZVZp(3),cplUhhVZpVZp(3),cplAhAhUhhUhh(3,3,3,3),cplUhhUhhhhhh(3,3,3,3),& 
& cplUhhUhhHpmcHpm(3,3,2,2),cplUhhUhhSdcSd(3,3,6,6),cplUhhUhhSecSe(3,3,6,6),             & 
& cplUhhUhhSucSu(3,3,6,6),cplUhhUhhSvcSv(3,3,6,6),cplUhhUhhcVWmVWm(3,3),cplUhhUhhVZVZ(3,3),& 
& cplUhhUhhVZpVZp(3,3)
...
Implicit None 
Real(dp), Intent(in) :: MAh(3),MAh2(3),Mhh(3),Mhh2(3),MCha(2),MCha2(2),MChi(6),MChi2(6),MFd(3),               & 
...
Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,mHd2,mHu2,ms2,vd,vu,vS,TW,TWp

Complex(dp), Intent(in) :: lam,Tlam

Complex(dp), Intent(in) :: cplUAhAhAh(3,3,3),cplUAhAhhh(3,3,3),cplcChaChaUAhL(2,2,3),cplcChaChaUAhR(2,2,3),      & 
& cplChiChiUAhL(6,6,3),cplChiChiUAhR(6,6,3),cplcFdFdUAhL(3,3,3),cplcFdFdUAhR(3,3,3),     & 
& cplcFeFeUAhL(3,3,3),cplcFeFeUAhR(3,3,3),cplcFuFuUAhL(3,3,3),cplcFuFuUAhR(3,3,3),       & 
& cplcFvFvUAhL(3,3,3),cplcFvFvUAhR(3,3,3),cplcgWmgWmUAh(3),cplcgWpCgWpCUAh(3),           & 
& cplUAhhhhh(3,3,3),cplUAhhhVZ(3,3),cplUAhhhVZp(3,3),cplUAhHpmcHpm(3,2,2),               & 
& cplUAhHpmcVWm(3,2),cplUAhSdcSd(3,6,6),cplUAhSecSe(3,6,6),cplUAhSucSu(3,6,6),           & 
& cplUAhSvcSv(3,6,6),cplUAhUAhAhAh(3,3,3,3),cplUAhUAhhhhh(3,3,3,3),cplUAhUAhHpmcHpm(3,3,2,2),& 
& cplUAhUAhSdcSd(3,3,6,6),cplUAhUAhSecSe(3,3,6,6),cplUAhUAhSucSu(3,3,6,6),               & 
& cplUAhUAhSvcSv(3,3,6,6),cplUAhUAhcVWmVWm(3,3),cplUAhUAhVZVZ(3,3),cplUAhUAhVZpVZp(3,3)
...
Real(dp), Intent(in) :: g1,g2,gp,QHd,QHu,Qs,mHd2,mHu2,vd,vu,vS
...
Complex(dp), Intent(in) :: lam,M1,M2,M4
...
& cplUChiFucSuL(6,3,6),cplUChiFucSuR(6,3,6),cplUChiFvcSvL(6,3,6),cplUChiFvcSvR(6,3,6),   & 
...
& cplcFvUChiSvL(3,6,6),cplcFvUChiSvR(3,6,6),cplcChaUChiVWmL(2,6),cplcChaUChiVWmR(2,6)
...
& MHpm2(2),MChi(6),MChi2(6),MVWm,MVWm2,MSu(6),MSu2(6),MFd(3),MFd2(3),MSv(6),             & 
...
Implicit None 
Real(dp), Intent(in) :: MFe(3),MFe2(3),MAh(3),MAh2(3),MSv(6),MSv2(6),MCha(2),MCha2(2),MSe(6),MSe2(6),         & 
...
Complex(dp), Intent(in) :: cplcUFeFeAhL(3,3,3),cplcUFeFeAhR(3,3,3),cplcUFeChaSvL(3,2,6),cplcUFeChaSvR(3,2,6),    & 
...
& cplSvcSvVZp(6,6),cplcVWmVWmVZp,cplAhAhVZpVZp(3,3),cplhhhhVZpVZp(3,3),cplHpmcHpmVZpVZp(2,2),& 
...
& cplcFuFuVZL(3,3),cplcFuFuVZR(3,3),cplcFvFvhhL(3,3,3),cplcFvFvhhR(3,3,3),               & 
& cplcFvFvVZL(3,3),cplcFvFvVZR(3,3),cplcgWmgWmhh(3),cplcgWmgWmVZ,cplcgWpCgWpChh(3),      & 
& cplcgWpCgWpCVZ,cplChiChihhL(6,6,3),cplChiChihhR(6,6,3),cplChiChiVZL(6,6),              & 
& cplChiChiVZR(6,6),cplcHpmVWmVZ(2),cplcVWmVWmVZ,cplhhcHpmVWm(3,2),cplhhcVWmVWm(3),      & 
& cplhhHpmcHpm(3,2,2),cplhhHpmcVWm(3,2),cplhhSdcSd(3,6,6),cplhhSecSe(3,6,6),             & 
& cplhhSucSu(3,6,6),cplhhSvcSv(3,6,6),cplHpmcHpmVZ(2,2),cplHpmcVWmVZ(2),cplSdcSdVZ(6,6), & 
...
& cplAhSvcSv(3,6,6),cplcChaChaAhL(2,2,3),cplcChaChaAhR(2,2,3),cplcChaChaVZL(2,2),        & 
& cplcChaChaVZR(2,2),cplcFdFdAhL(3,3,3),cplcFdFdAhR(3,3,3),cplcFdFdVZL(3,3),             & 
...
& cplcFuFuVZR(3,3),cplcFvFvAhL(3,3,3),cplcFvFvAhR(3,3,3),cplcFvFvVZL(3,3),               & 
& cplcFvFvVZR(3,3),cplcgWmgWmAh(3),cplcgWmgWmVZ,cplcgWpCgWpCAh(3),cplcgWpCgWpCVZ,        & 
& cplChiChiAhL(6,6,3),cplChiChiAhR(6,6,3),cplChiChiVZL(6,6),cplChiChiVZR(6,6),           & 
& cplcHpmVWmVZ(2),cplhhVZVZ(3),cplhhVZVZp(3),cplHpmcHpmVZ(2,2),cplHpmcVWmVZ(2),          & 
...
Complex(dp), Intent(in) :: cplAhhhhh(3,3,3),cplAhhhVZp(3,3),cplcChaChahhL(2,2,3),cplcChaChahhR(2,2,3),           & 
...
& cplcFuFuVZpL(3,3),cplcFuFuVZpR(3,3),cplcFvFvhhL(3,3,3),cplcFvFvhhR(3,3,3),             & 
& cplcFvFvVZpL(3,3),cplcFvFvVZpR(3,3),cplcgWmgWmhh(3),cplcgWmgWmVZp,cplcgWpCgWpChh(3),   & 
& cplcgWpCgWpCVZp,cplChiChihhL(6,6,3),cplChiChihhR(6,6,3),cplChiChiVZpL(6,6),            & 
& cplChiChiVZpR(6,6),cplcHpmVWmVZp(2),cplcVWmVWmVZp,cplhhcHpmVWm(3,2),cplhhcVWmVWm(3),   & 
& cplhhHpmcHpm(3,2,2),cplhhHpmcVWm(3,2),cplhhSdcSd(3,6,6),cplhhSecSe(3,6,6),             & 
& cplhhSucSu(3,6,6),cplhhSvcSv(3,6,6),cplHpmcHpmVZp(2,2),cplHpmcVWmVZp(2),               & 
...
Complex(dp), Intent(in) :: cplAhAhhh(3,3,3),cplAhcHpmVWm(3,2),cplAhhhVZ(3,3),cplAhhhVZp(3,3),cplAhHpmcHpm(3,2,2),& 
& cplAhHpmcVWm(3,2),cplAhSdcSd(3,6,6),cplAhSecSe(3,6,6),cplAhSucSu(3,6,6),               & 
& cplAhSvcSv(3,6,6),cplcChaChaAhL(2,2,3),cplcChaChaAhR(2,2,3),cplcChaChaVZpL(2,2),       & 
& cplcChaChaVZpR(2,2),cplcFdFdAhL(3,3,3),cplcFdFdAhR(3,3,3),cplcFdFdVZpL(3,3),           & 
& cplcFdFdVZpR(3,3),cplcFeFeAhL(3,3,3),cplcFeFeAhR(3,3,3),cplcFeFeVZpL(3,3),             & 
& cplcFeFeVZpR(3,3),cplcFuFuAhL(3,3,3),cplcFuFuAhR(3,3,3),cplcFuFuVZpL(3,3),             & 
& cplcFuFuVZpR(3,3),cplcFvFvAhL(3,3,3),cplcFvFvAhR(3,3,3),cplcFvFvVZpL(3,3),             & 
& cplcFvFvVZpR(3,3),cplcgWmgWmAh(3),cplcgWmgWmVZp,cplcgWpCgWpCAh(3),cplcgWpCgWpCVZp,     & 
& cplChiChiAhL(6,6,3),cplChiChiAhR(6,6,3),cplChiChiVZpL(6,6),cplChiChiVZpR(6,6),         & 
& cplcHpmVWmVZp(2),cplhhVZpVZp(3),cplhhVZVZp(3),cplHpmcHpmVZp(2,2),cplHpmcVWmVZp(2),     & 
& cplSdcSdVZp(6,6),cplSecSeVZp(6,6),cplSucSuVZp(6,6),cplSvcSvVZp(6,6)
...
End Module LoopMasses_NonUMSSMv4 
